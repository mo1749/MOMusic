...
const DEFAULT_SPOTIFY_MARKET = String(process.env.MOMusic_SPOTIFY_MARKET || process.env.SPOTIFY_MARKET || 'US').trim().toUpperCase();
...
function spotifyConfigFileCandidates() {
  ...
  add(firstEnv(['SPOTIFY_CONFIG_FILE', 'MOMusic_SPOTIFY_CONFIG_FILE']));
...
function getSpotifyConfigFile() {
  return process.env.SPOTIFY_CONFIG_FILE || process.env.MOMusic_SPOTIFY_CONFIG_FILE || DEFAULT_SPOTIFY_CONFIG_FILE;
}
...
function getSpotifyTokenFile() {
  return process.env.SPOTIFY_TOKEN_FILE || process.env.MOMusic_SPOTIFY_TOKEN_FILE || DEFAULT_SPOTIFY_TOKEN_FILE;
}
...
function getSpotifyOAuthConfig() {
  ...
  const envClientId = firstEnv(['SPOTIFY_CLIENT_ID', 'MOMusic_SPOTIFY_CLIENT_ID']);
  const envClientSecret = firstEnv(['SPOTIFY_CLIENT_SECRET', 'MOMusic_SPOTIFY_CLIENT_SECRET']);
  const envRedirectUri = firstEnv(['SPOTIFY_REDIRECT_URI', 'MOMusic_SPOTIFY_REDIRECT_URI']);
  const envScopes = normalizeScopes(firstEnv(['SPOTIFY_SCOPES', 'SPOTIFY_SCOPE', 'MOMusic_SPOTIFY_SCOPES']));
  ...
  // MOMusic owns the OAuth flow, so always include the scopes required by
  ...
  const market = (firstEnv(['SPOTIFY_MARKET', 'MOMusic_SPOTIFY_MARKET']) || fileConfig.market || DEFAULT_SPOTIFY_MARKET || 'US').toUpperCase();
  const missing = [];
...
function spotifyErrorDetails(err) {
  ...
  if (reauthRequired || statusCode === 401 || code === 'SPOTIFY_REFRESH_TOKEN_MISSING') {
    ...
    message = 'Spotify 服务暂时不可用，MOMusic 已完成有限重试，请稍后再试。';
...
async function handleSpotifyCreatePlaylist(name, opts) {
  ...
  const created = await spotifyUserRequest('/me/playlists', 'POST', {}, {
    ...
    public: isPublic,
    collaborative: false,
    description: normalizeText(opts.description || 'Created with MOMusic'),
  }, { timeoutMs: 10000 });
  return {
    provider: 'spotify',
    loggedIn: true,
...
async function handleSpotifySongUrl(track) {
  ...
  return {
    ...
    restriction: {
      category: 'provider_limited',
      reason: 'spotify_metadata_only',
      message: 'Spotify 官方 Web API 不提供可交给 MOMusic 播放的音频直链，正在自动换源。',
      action: 'switch_source',
    },
  };
}
...
async function handleSpotifyLyric(id) {
  return {
    ...
    ytlrc: '',
    source: 'none',
    message: 'Spotify Web API 不提供歌词，MOMusic 会沿用跨平台歌词兜底。',
  };
}
