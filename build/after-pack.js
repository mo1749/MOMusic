...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for MOMusic icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'MOMusic';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`MOMusic executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`MOMusic icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting MOMusic resources  rcedit=${rceditPath}`);
