...
const SYSTEM_PURGE_AVAILABLE = isWin && process.env.MOMusic_DISABLE_SYSTEM_MEMORY_PURGE !== '1';
const SYSTEM_PURGE_ENABLED = SYSTEM_PURGE_AVAILABLE && process.env.MOMusic_DISABLE_AUTOMATIC_SYSTEM_MEMORY_PURGE !== '1';
...
const NATIVE_TYPE_BLOCK = [
  ...
  '}',
  'public static class MOMusicMemNative {',
...
function defaultNativeTempPath() {
  const configured = String(process.env.MOMusic_NATIVE_TEMP_DIR || '').trim();
  if (configured) return path.resolve(configured);
  const localRoot = String(process.env.LOCALAPPDATA || process.env.APPDATA || os.tmpdir()).trim();
  return path.join(localRoot, 'MOMusic', 'native-helper-temp');
}
...
function makeTempPath(label, ext) {
  return path.join(ensureNativeTempPath(), 'MOMusic-' + label + '-' + process.pid + '-' + Date.now() + '.' + ext);
}
  ...
  return new Promise((resolve, reject) => {
    execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath], {
      ...
      env: {
        ...
        MOMusic_NATIVE_TEMP_DIR: ensureNativeTempPath(),
      },
    }, (error, stdout, stderr) => {
...
function buildPurgeScript(mask, resultPath) {
  ...
  const lines = [
    NATIVE_TYPE_BLOCK,
    '[MOMusicMemNative]::PreparePrivileges() | Out-Null',
    '$mask = ' + m,
    '$before = [MOMusicMemNative]::GetUsedPhys()',
    '$loadBefore = [MOMusicMemNative]::GetMemoryLoad()',
    '$steps = @()',
  ];

  if ((m & MEMORY_MASK.workingSet) !== 0) {
    lines.push('$steps += @{ id="workingSet"; status=[MOMusicMemNative]::PurgeList(' + MEMORY_CMD.emptyWorkingSets + ') }');
    lines.push('$steps += @{ id="systemFileCache"; status=[MOMusicMemNative]::FlushSystemFileCache() }');
  }
  if ((m & MEMORY_MASK.modifiedList) !== 0) {
    lines.push('$steps += @{ id="modifiedList"; status=[MOMusicMemNative]::PurgeList(' + MEMORY_CMD.flushModifiedList + ') }');
  }
  if ((m & MEMORY_MASK.standbyList) !== 0) {
    lines.push('$steps += @{ id="standbyList"; status=[MOMusicMemNative]::PurgeList(' + MEMORY_CMD.purgeStandbyList + ') }');
  }
  if ((m & MEMORY_MASK.standbyLow) !== 0) {
    lines.push('$steps += @{ id="standbyLow"; status=[MOMusicMemNative]::PurgeList(' + MEMORY_CMD.purgeStandbyLow + ') }');
  }

  lines.push('$after = [MOMusicMemNative]::GetUsedPhys()');
  lines.push('$loadAfter = [MOMusicMemNative]::GetMemoryLoad()');
...
function probeProcessElevation() {
  ...
  const scriptPath = writeTempScript('elev-check', [
    NATIVE_TYPE_BLOCK,
    'Write-Output ([MOMusicMemNative]::IsTokenElevated() | ConvertTo-Json -Compress)',
  ]);
...
function purgeSystemMemoryElevated(mask, options) {
  ...
  if (!SYSTEM_PURGE_AVAILABLE || (!SYSTEM_PURGE_ENABLED && options.manual !== true)) {
    return Promise.resolve({
      ...
      disabled: true,
      needAdmin: false,
      message: 'Elevated memory purge is disabled by default; MOMusic will not open administrator PowerShell windows.',
    });
  }
  if (!isWin) {
    return Promise.resolve({ ok: false, unsupported: true, message: 'System memory purge is Windows-only.' });
...
function queryExtendedMemoryStats() {
  ...
  const scriptPath = writeTempScript('mem-stats', [
    NATIVE_TYPE_BLOCK,
    '$total = [MOMusicMemNative]::GetTotalPhys()',
    '$avail = [MOMusicMemNative]::GetAvailPhys()',
    '$load = [MOMusicMemNative]::GetMemoryLoad()',
...
function trimAppWorkingSets(pids) {
  ...
  const scriptPath = writeTempScript('mem-trim', [
    ...
    'public static class MOMusicTrim {',
    ...
    '\'@',
    '$pids = @(' + pidLiteral + ')',
    '$trimmed = [MOMusicTrim]::TrimMany([int[]]$pids)',
