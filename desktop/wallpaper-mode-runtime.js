...
function normalizeWallpaperState(previous, payload, enabledOverride) {
  ...
  return {
    enabled,
    title: String(Object.prototype.hasOwnProperty.call(source, 'title') ? source.title : current.title || 'MOMusic').slice(0, 512),
...
if (-not ("MOMusicDesktopWallpaperNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class MOMusicDesktopWallpaperNative {
...
try { $previousDpiContext = [MOMusicDesktopWallpaperNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
$progman = [MOMusicDesktopWallpaperNative]::FindWindowByClass("Progman")
if ($progman -eq [IntPtr]::Zero) { throw "WALLPAPER_PROGMAN_NOT_FOUND" }
$sendResult = [IntPtr]::Zero
[MOMusicDesktopWallpaperNative]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$sendResult) | Out-Null
$script:workerw = [IntPtr]::Zero
$callback = [MOMusicDesktopWallpaperNative+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$state)
  $shellView = [MOMusicDesktopWallpaperNative]::FindWindowExByClass($top, [IntPtr]::Zero, "SHELLDLL_DefView")
  if ($shellView -ne [IntPtr]::Zero) {
    $candidate = [MOMusicDesktopWallpaperNative]::FindWindowExByClass([IntPtr]::Zero, $top, "WorkerW")
    if ($candidate -ne [IntPtr]::Zero) { $script:workerw = $candidate }
  }
  return $true
}
[MOMusicDesktopWallpaperNative]::EnumWindows($callback, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { throw "WALLPAPER_WORKERW_NOT_FOUND" }
$target = [IntPtr]::new([Int64]${hwnd})
if (-not [MOMusicDesktopWallpaperNative]::IsWindow($target)) { throw "WALLPAPER_TARGET_NOT_FOUND" }
$GWL_STYLE = -16
$WS_POPUP = [Int64]0x80000000
$WS_CHILD = [Int64]0x40000000
$style = [MOMusicDesktopWallpaperNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
$childStyle = ($style -band (-bnot $WS_POPUP)) -bor $WS_CHILD
[MOMusicDesktopWallpaperNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$childStyle)) | Out-Null
$verifiedStyle = [MOMusicDesktopWallpaperNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
if (($verifiedStyle -band $WS_CHILD) -eq 0 -or ($verifiedStyle -band $WS_POPUP) -ne 0) { throw "WALLPAPER_CHILD_STYLE_FAILED" }
[MOMusicDesktopWallpaperNative]::SetParent($target, $script:workerw) | Out-Null
$parent = [MOMusicDesktopWallpaperNative]::GetParent($target)
if ($parent -ne $script:workerw) { throw "WALLPAPER_WORKERW_ATTACH_FAILED" }
$origin = New-Object MOMusicDesktopWallpaperNative+POINT
$origin.X = ${x}
$origin.Y = ${y}
if (-not [MOMusicDesktopWallpaperNative]::ScreenToClient($script:workerw, [ref]$origin)) { throw "WALLPAPER_WORKERW_BOUNDS_FAILED" }
$positioned = [MOMusicDesktopWallpaperNative]::SetWindowPos($target, [IntPtr]::new([Int64]1), $origin.X, $origin.Y, ${width}, ${height}, 0x0030)
if (-not $positioned) { throw "WALLPAPER_WORKERW_POSITION_FAILED" }
$className = New-Object System.Text.StringBuilder 128
[MOMusicDesktopWallpaperNative]::GetClassName($script:workerw, $className, $className.Capacity) | Out-Null
[pscustomobject]@{
  ok = $true
...
if ($previousDpiContext -ne [IntPtr]::Zero) {
  try { [MOMusicDesktopWallpaperNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
}
`;
}
...
class DesktopWallpaperRuntime {
  ...
  sendState(win = this.window) {
    ...
    win.webContents.send('MOMusic-wallpaper-state', this.state);
    return true;
  }
  ...
  createWindow() {
    ...
    const win = new this.BrowserWindow({
      ...
      skipTaskbar: true,
      show: false,
      title: 'MOMusic Desktop Wallpaper',
      webPreferences: {
