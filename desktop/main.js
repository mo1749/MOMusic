...
const APP_METADATA = APP_PACKAGE_INFO.MOMusic || {};
const APP_NAME = process.env.MOMusic_RUNTIME_NAME || APP_METADATA.runtimeName || APP_PACKAGE_INFO.productName || 'MOMusic';
const APP_USER_MODEL_ID = process.env.MOMusic_APP_USER_MODEL_ID || APP_METADATA.appUserModelId || (APP_PACKAGE_INFO.build && APP_PACKAGE_INFO.build.appId) || 'com.MOMusic.desktop';
...
const NETEASE_LOGIN_PARTITION = 'persist:MOMusic-netease-login';
...
const QISHUI_LOGIN_PARTITION = 'persist:MOMusic-qishui-oauth-login';
const SPOTIFY_LOGIN_PARTITION = 'persist:MOMusic-spotify-login';
...
process.env.MOMusic_NATIVE_TEMP_DIR = NATIVE_HELPER_TEMP_PATH;
...
const MAIN_WINDOW_BACKGROUND_THROTTLING = process.env.MOMusic_KEEP_BACKGROUND_RENDERING === '1' ? false : true;
...
async function cacheSettingsSnapshot() {
  ...
  const activeBeatmapsPath = process.env.MOMusic_BEAT_CACHE_DIR || settings.beatmapsPath;
  const activeUpdatesPath = process.env.MOMusic_UPDATE_DIR || settings.updatesPath;
...
const CHROMIUM_OPT_IN_PERFORMANCE_SWITCHES = [
  ['ignore-gpu-blocklist', null, 'MOMusic_IGNORE_GPU_BLOCKLIST'],
  ['force_high_performance_gpu', null, 'MOMusic_FORCE_HIGH_PERFORMANCE_GPU'],
  ['disable-background-timer-throttling', null, 'MOMusic_KEEP_BACKGROUND_RENDERING'],
  ['disable-renderer-backgrounding', null, 'MOMusic_KEEP_BACKGROUND_RENDERING'],
  ['disable-backgrounding-occluded-windows', null, 'MOMusic_KEEP_BACKGROUND_RENDERING'],
];
...
function withStartupTimeout(promise, timeoutMs, label, onTimeout) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      error.code = 'MOMusic_STARTUP_TIMEOUT';
      reject(error);
...
function waitForServer(server, timeoutMs = STARTUP_SERVER_TIMEOUT_MS) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      error.code = 'MOMusic_SERVER_TIMEOUT';
      finish(error);
  ...
  return new Promise((resolve, reject) => {
    ...
    const probe = () => {
      ...
      if (Date.now() >= deadline) {
        ...
        error.code = 'MOMusic_HTTP_TIMEOUT';
        finish(error);
        return;
      }
...
function broadcastDesktopWallpaperStatus(status) {
  ...
  mainWindow.webContents.send('MOMusic-wallpaper-runtime-state', {
...
async function prepareWallpaperEngineRendererCapture(sessionId, fps) {
  ...
  const script = `(() => {
    const prepare = window.__MOMusicPrepareWallpaperEngineCapture;
...
async function prepareWallpaperEngineRendererGlassCapture(sessionId, fps, sourceId) {
  ...
  const script = `(() => {
    const prepare = window.__MOMusicPrepareWallpaperEngineGlassCapture;
...
async function prepareWallpaperEngineRendererHostBoundsFrame(sessionId, reason = 'bounds-changed') {
  ...
  const script = `(() => {
    const prepare = window.__MOMusicPrepareWallpaperEngineHostBoundsChange;
...
async function prepareWallpaperEngineRendererDesktopPreview(sessionId, reason = 'full-desktop-passive') {
  ...
  const script = `(() => {
    const prepare = window.__MOMusicPrepareWallpaperEngineDesktopPreview;
...
function suspendWallpaperEngineForHiddenHost(win, reason = 'hidden') {
  ...
  cancelWallpaperEngineHostBoundsRestart();
  try {
    win.webContents.send('MOMusic-wallpaper-engine-host-bounds-changed', {
      phase: 'prepare',
...
function configureLocalAppPermissions() {
  ...
  if (!ses || ses._MOMusicPermissionsConfigured) return;
  ses._MOMusicPermissionsConfigured = true;
    ...
    Promise.resolve().then(async () => {
      ...
      if (grant.kind === 'dwm-glass') {
        ...
        if (wallpaperEngineCaptureGrant !== grant
          ...
          || String(source && source.name || '') !== 'MOMusic WE DWM Surface') {
          reply({});
          return;
        }
        reply({ video: source });
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('MOMusic-global-hotkey', { action });
}

function unregisterMOMusicGlobalHotkeys() {
...
function configureMOMusicGlobalHotkeys(bindings = []) {
  unregisterMOMusicGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
async function getGpuDiagnostics() {
  ...
  return {
    ...
    switches: {
      ...
      ignoreGpuBlocklist: process.env.MOMusic_IGNORE_GPU_BLOCKLIST === '1',
      forceHighPerformanceGpu: process.env.MOMusic_FORCE_HIGH_PERFORMANCE_GPU === '1',
      keepBackgroundRendering: process.env.MOMusic_KEEP_BACKGROUND_RENDERING === '1',
      angle: 'd3d11',
    },
  };
}
...
function reportWindowCreationFailure(context, error) {
  ...
  if (!startupErrorReported) {
    ...
    try {
      ...
      // dialog.showErrorBox('MOMusic 启动失败'
      dialog.showErrorBox(`MOMusic 启动失败 (${code})`, buildStartupErrorMessage(context, code, logInfo, error));
    } catch (_) {}
  }
  if (!startupCompleted) {
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.MOMusic_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.MOMusic_CREATE_DESKTOP_SHORTCUT === '1';
}
  ...
  return new Promise((resolve) => {
    ...
    const buildQrHtml = (statusText) => {
      ...
      return [
        ...
        '<h1>使用汽水音乐 App 扫码</h1>',
        '<p>请用汽水音乐 App 扫码并确认。确认后 MOMusic 会自动保存汽水登录态，同步汽水歌单与我的喜欢。</p>',
...
async function openQishuiMusicLoginWindow(owner) {
  ...
  if (imported && imported.cookie) {
    ...
    return {
      ...
      message: '已读取本机汽水 PC 登录态，正在导入 MOMusic 并同步我的喜欢和歌单',
    };
  }

  // Reuse MOMusic's last imported copy only when the official client data is
  ...
  if (saved && saved.cookie && savedStatus && savedStatus.loggedIn && savedStatus.webSession) {
    return {
      ...
      message: imported && imported.locked
        ? '本机汽水登录数据库暂时被占用，已继续使用 MOMusic 上次导入的有效登录态。'
        : '已继续使用 MOMusic 上次从本机汽水 PC 导入的登录态。',
    };
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const callbackServer = http.createServer(async (req, res) => {
      ...
      try {
        ...
        res.end(spotifyOAuthResultHtml(ok, (result && (result.message || result.error)) || (ok ? '可以回到 MOMusic。' : '请回到 MOMusic 重新尝试。')));
      } catch (e) {
...
public class MOMusicMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([MOMusicMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  const locked = desktopLyricsState.clickThrough !== false;
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('MOMusic-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('MOMusic-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('MOMusic-desktop-lyrics-state', desktopLyricsState);
}
...
function hookExplorerRestartForFullDesktop(win) {
  ...
  if (win.__MOMusicTaskbarCreatedHookPending || win.__MOMusicTaskbarCreatedMessageId) return;
  win.__MOMusicTaskbarCreatedHookPending = true;
  const script = `
Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;
public static class MOMusicShellMessage {
  [DllImport("user32.dll", CharSet=CharSet.Unicode)]
  public static extern uint RegisterWindowMessage(string messageName);
}
"@
[MOMusicShellMessage]::RegisterWindowMessage("TaskbarCreated")
`;
  execFile('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', script], {
    ...
    win.__MOMusicTaskbarCreatedHookPending = false;
    ...
    try {
      ...
      win.__MOMusicTaskbarCreatedMessageId = messageId;
    } catch (hookError) {
...
ipcMain.on('MOMusic-full-desktop-icon-shields', (event, payload = {}) => {
...
ipcMain.handle('MOMusic-full-desktop-set-icons-visible', async (event, visible) => {
...
ipcMain.handle('MOMusic-full-desktop-set-software-lock', async (event, locked) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'DESKTOP_MODE_UNTRUSTED_SENDER' };
  return fullDesktopModeRuntime.setSoftwareInteractionLocked(locked === true, 'renderer-software-lock');
});

ipcMain.on('MOMusic-full-desktop-request-keyboard-focus', (event, reason) => {
  if (!isTrustedMainWindowIpc(event)) return;
...
ipcMain.on('MOMusic-full-desktop-pointer-route', (event, payload = {}) => {
  if (!isTrustedMainWindowIpc(event)) return;
...
ipcMain.handle('MOMusic-get-gpu-diagnostics', () => {
  return getGpuDiagnostics();
});

ipcMain.handle('MOMusic-memory-get-snapshot', async () => {
  try {
    return {
      ok: true,
...
ipcMain.handle('MOMusic-memory-configure-auto', async (_event, payload = {}) => {
...
ipcMain.handle('MOMusic-memory-trim-app', async (_event, payload = {}) => {
  return trimAppMemoryNow(payload.reason || 'renderer');
});

ipcMain.handle('MOMusic-memory-purge-system', async (_event, payload = {}) => {
  ...
  try {
    if (isMainWindowForegroundVisible()) {
      return {
        ok: true,
        result: { ok: false, skipped: true, reason: 'foreground-visible', message: 'System memory purge is skipped while MOMusic is visible.' },
...
ipcMain.handle('MOMusic-cache-get-settings', async () => {
  try {
...
ipcMain.handle('MOMusic-cache-choose-directory', async () => {
  const result = await dialog.showOpenDialog({
    title: '选择 MOMusic 缓存目录',
...
ipcMain.handle('MOMusic-cache-set-settings', async (_event, payload = {}) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-engine-list', async (event, payload = {}) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-engine-project-details', async (event, id) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-engine-open-project-details', async (event, payload = {}) => {
  try {
    if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
...
ipcMain.handle('MOMusic-wallpaper-engine-choose-directory', async (event) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-engine-choose-project-file', async (event) => {
  try {
    if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, canceled: false, projects: [], count: 0, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
    const options = {
...
ipcMain.handle('MOMusic-wallpaper-engine-remove-directory', async (event, rootId) => {
  try {
    if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, projects: [], count: 0, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
...
ipcMain.handle('MOMusic-wallpaper-engine-runtime-status', async (event, payload = {}) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-engine-start-scene', async (event, payload = {}) => {
  let operation = 0;
...
ipcMain.handle('MOMusic-wallpaper-engine-capture-result', async (event, payload = {}) => {
  if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
...
ipcMain.handle('MOMusic-wallpaper-engine-prepare-glass-capture', async (event, payload = {}) => {
  if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
  const sessionId = String(payload && payload.sessionId || '');
  if (!/^[a-f0-9]{24}$/i.test(sessionId)) return { ok: false, error: 'WALLPAPER_ENGINE_SESSION_INVALID' };
...
ipcMain.handle('MOMusic-wallpaper-engine-activate-dwm-surface', async (event, payload = {}) => {
  if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
  const sessionId = String(payload && payload.sessionId || '');
  if (!/^[a-f0-9]{24}$/i.test(sessionId)) return { ok: false, error: 'WALLPAPER_ENGINE_SESSION_INVALID' };
  try {
...
ipcMain.on('MOMusic-wallpaper-engine-glass-surface', (event, payload = {}) => {
...
ipcMain.on('MOMusic-wallpaper-engine-pointer-activity', (event, payload = {}) => {
...
ipcMain.handle('MOMusic-wallpaper-engine-stop-scene', async (event, payload = {}) => {
  try {
    if (!isTrustedWallpaperEngineIpc(event)) return { ok: false, error: 'WALLPAPER_ENGINE_UNTRUSTED_CALLER' };
...
ipcMain.handle('MOMusic-cache-read-lyric', async (_event, key) => {
  try {
...
ipcMain.handle('MOMusic-cache-write-lyric', async (_event, key, payload) => {
  try {
...
ipcMain.handle('MOMusic-hotkeys-configure-global', (_event, bindings) => {
  return configureMOMusicGlobalHotkeys(bindings);
});
...
function loginCookieExportMeta(provider) {
  ...
  const entries = {
    ...
    'kugou-concept': { label: '酷狗概念版', files: [process.env.KUGOU_CONCEPT_COOKIE_FILE, path.join(userData, '.kugou-concept-cookie')] },
...
ipcMain.handle('MOMusic-export-login-cookie', async (_event, provider) => {
  try {
...
ipcMain.handle('MOMusic-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'MOMusic-export.json').replace(/[\\/:*?"<>|]+/g, '-');
...
ipcMain.handle('MOMusic-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
...
ipcMain.on('MOMusic-current-fx-autosave-read-sync', (event) => {
  event.returnValue = { ok: true, payload: readCurrentFxAutosaveFile() };
});

ipcMain.on('MOMusic-current-fx-autosave-save-sync', (event, payload) => {
  event.returnValue = writeCurrentFxAutosaveFile(payload || {});
});

ipcMain.handle('MOMusic-current-fx-autosave-save', async (_event, payload = {}) => {
  return writeCurrentFxAutosaveFile(payload || {});
});

ipcMain.handle('MOMusic-login-easter-egg-status', async (event) => {
...
ipcMain.handle('MOMusic-login-easter-egg-unlock', async (event, value) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'UNTRUSTED_SENDER', unlocked: false };
  return loginEasterEggGate.unlock(value);
});

ipcMain.handle('MOMusic-login-easter-egg-reset', async (event) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'UNTRUSTED_SENDER', unlocked: false };
...
// ── 一起听 (Listen Together) IPC ──

ipcMain.handle('listen-together-wss', async () => {
  const { wss, rooms, clients } = require('../listen-together');
  return {
    ok: true,
    wssActive: !!wss,
    rooms: rooms.size,
...
ipcMain.handle('listen-together-list-rooms', async () => {
  const { rooms, MSG } = require('../listen-together');
  const roomList = [];
  rooms.forEach((room) => {
    roomList.push(room.info);
  });
  return { ok: true, rooms: roomList };
});

ipcMain.handle('MOMusic-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('MOMusic-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('MOMusic-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('MOMusic-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('MOMusic-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('MOMusic-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('MOMusic-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('MOMusic-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('MOMusic-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-set-enabled', async (event, enabled, payload) => {
  try {
...
ipcMain.handle('MOMusic-wallpaper-update', async (event) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, enabled: false, error: 'WALLPAPER_UNTRUSTED_SENDER' };
  const status = {
...
ipcMain.handle('MOMusic-wallpaper-get-status', async (event) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, enabled: false, error: 'WALLPAPER_UNTRUSTED_SENDER' };
  return {
    ok: true,
    status: {
...
function configureLocalServerEnvironment(port) {
  ...
  process.env.MOMusic_LISTEN_SYNC_FILE = path.join(STABLE_USER_DATA_PATH, 'listen-sync-journal.json');
  process.env.MOMusic_LOGIN_EASTER_EGG_GATE_FILE = path.join(STABLE_USER_DATA_PATH, LOGIN_EASTER_EGG_STATE_FILE);
  process.env.MOMusic_LOGIN_EASTER_EGG_GATE_VERSION = LOGIN_EASTER_EGG_GATE_VERSION;
  ...
  if (!process.env.SPOTIFY_CONFIG_FILE && !process.env.MOMusic_SPOTIFY_CONFIG_FILE) {
    process.env.SPOTIFY_CONFIG_FILE = path.join(STABLE_USER_DATA_PATH, '.spotify-credentials.json');
  }
  process.env.MOMusic_UPDATE_DIR = getUpdateDownloadDir();
}
...
async function ensureLocalServerStarted() {
  ...
  localServerStartPromise = (async () => {
    const injectedDelay = Math.max(0, Math.min(15000, Number(process.env.MOMusic_STARTUP_TEST_SERVER_DELAY_MS) || 0));
    ...
    try {
      const ltWss = startListenTogether(localServer);
      if (ltWss) {
        console.log('[ListenTogether] 一起听服务已就绪');
      }
    } catch (ltErr) {
      console.warn('[ListenTogether] 启动失败（非致命）:', ltErr.message);
    }
...
function showMainWindowSafely(win, reason) {
  ...
  if (win.__MOMusicStartupShowTimer) {
    clearTimeout(win.__MOMusicStartupShowTimer);
    win.__MOMusicStartupShowTimer = null;
  }
  ensureMainWindowInsideDisplay(win);
  if (win.isMinimized()) win.restore();
  if (!win.isVisible()) win.show();
...
async function loadMainWindowWithRetry(win) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      if (attempt === 1 && process.env.MOMusic_STARTUP_TEST_FAIL_FIRST_NAV === '1') {
        const injected = new Error('Injected first navigation failure for startup QA');
        injected.code = 'MOMusic_STARTUP_QA_INJECTED';
        throw injected;
      }
  ...
  error.code = (lastError && lastError.code) || 'MOMusic_NAVIGATION_FAILED';
  throw error;
}
...
async function createWindowOnce() {
  ...
  const win = new BrowserWindow({
    ...
    resizable: true,
    transparent: true,
    opacity: process.env.MOMusic_STARTUP_QA_HIDDEN === '1' ? 0 : 1,
    backgroundColor: '#00000000',
    hasShadow: true,
    autoHideMenuBar: true,
    title: APP_NAME,
  ...
  win.__MOMusicStartupShowTimer = setTimeout(() => {
  ...
  win.on('close', (event) => {
    ...
    if (desktopMode.enabled === true) {
      event.preventDefault();
      if (win.__MOMusicDesktopModeCloseArmed) return;
      win.__MOMusicDesktopModeCloseArmed = true;
      disableFullDesktopMode('main-window-close').then((result) => {
        if (result && result.ok === true) {
          if (!win.isDestroyed()) win.close();
          return;
        }
        win.__MOMusicDesktopModeCloseArmed = false;
        console.warn(
  ...
  const qaExitMs = Math.max(0, Math.min(10000, Number(process.env.MOMusic_STARTUP_QA_EXIT_MS) || 0));
  if (qaExitMs) {
    setTimeout(() => {
      appQuitting = true;
      app.quit();
