...
function signatureScript() {
  const source = [
    ...
    "$target = [Environment]::GetEnvironmentVariable('MOMusic_WE_SIGNATURE_TARGET', 'Process')",
...
$target = [Environment]::GetEnvironmentVariable('MOMusic_WE_ENGINE_TARGET', 'Process')
$expected = ''
...
function controlBrokerScript() {
  const source = String.raw`
$ErrorActionPreference = 'Stop'
$target = [Environment]::GetEnvironmentVariable('MOMusic_WE_CONTROL_TARGET', 'Process')
$commandLine = [Environment]::GetEnvironmentVariable('MOMusic_WE_CONTROL_COMMAND_LINE', 'Process')
$waitForExit = [Environment]::GetEnvironmentVariable('MOMusic_WE_CONTROL_WAIT', 'Process') -eq '1'
$waitTimeout = 10000
try { $waitTimeout = [Math]::Max(1000, [Math]::Min(20000, [int][Environment]::GetEnvironmentVariable('MOMusic_WE_CONTROL_WAIT_TIMEOUT', 'Process'))) } catch { $waitTimeout = 10000 }
...
public static class MOMusicExplorerParentLauncher {
...
[void][MOMusicExplorerParentLauncher]::Launch($target, $commandLine, $workingDirectory, $waitForExit, $waitTimeout)
`.trim();
  return Buffer.from(source, 'utf16le').toString('base64');
}
...
function nativeWindowControlScript() {
  const source = String.raw`
$ErrorActionPreference = 'Stop'
$action = [Environment]::GetEnvironmentVariable('MOMusic_WE_WINDOW_ACTION', 'Process')
$sourceId = [Environment]::GetEnvironmentVariable('MOMusic_WE_WINDOW_SOURCE_ID', 'Process')
$expectedTitle = [Environment]::GetEnvironmentVariable('MOMusic_WE_WINDOW_TITLE', 'Process')
$expectedExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_WINDOW_EXECUTABLE', 'Process')
$hostWindowId = [Environment]::GetEnvironmentVariable('MOMusic_WE_HOST_WINDOW_ID', 'Process')
$hostExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_HOST_EXECUTABLE', 'Process')
$hostCornerRadius = [Environment]::GetEnvironmentVariable('MOMusic_WE_HOST_CORNER_RADIUS', 'Process')
...
public sealed class MOMusicWeWindowResult {
...
public static class MOMusicWeWindowControl {
  ...
  static MOMusicWeWindowResult Snapshot(IntPtr hWnd, uint processId) {
    RECT rect;
    ...
    return new MOMusicWeWindowResult {
      ok = true,
      left = rect.Left,
      top = rect.Top,
      right = rect.Right,
  ...
  static MOMusicWeWindowResult RunDpiAware(string action, string sourceId, string expectedTitle, string expectedExecutable, string hostWindowId, string hostExecutable, string hostCornerRadius) {
    IntPtr hWnd = ParseHandle(sourceId);
    if (!IsWindow(hWnd)) return new MOMusicWeWindowResult { ok = true, missing = true };
    ...
    if (String.Equals(action, "close", StringComparison.OrdinalIgnoreCase)) {
      MOMusicWeWindowResult closeResult = Snapshot(hWnd, processId);
    ...
    if (String.Equals(action, "park", StringComparison.OrdinalIgnoreCase)) {
      ...
      MOMusicWeWindowResult parkResult = Snapshot(hWnd, processId);
    ...
    MOMusicWeWindowResult result = Snapshot(hWnd, processId);
    result.moved = false;
  ...
  public static MOMusicWeWindowResult Run(string action, string sourceId, string expectedTitle, string expectedExecutable, string hostWindowId, string hostExecutable, string hostCornerRadius) {
...
'@
Add-Type -TypeDefinition $source -Language CSharp
[MOMusicWeWindowControl]::Run($action, $sourceId, $expectedTitle, $expectedExecutable, $hostWindowId, $hostExecutable, $hostCornerRadius) | ConvertTo-Json -Compress
`.trim();
  return Buffer.from(source, 'utf16le').toString('base64');
}
...
function nativeParallaxPointerRelayScript() {
  const source = String.raw`
$ErrorActionPreference = 'Stop'
$sourceId = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_SOURCE_ID', 'Process')
$expectedTitle = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_SOURCE_TITLE', 'Process')
$expectedExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_SOURCE_EXECUTABLE', 'Process')
$hostWindowId = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_HOST_WINDOW_ID', 'Process')
$hostExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_HOST_EXECUTABLE', 'Process')
$sessionId = [Environment]::GetEnvironmentVariable('MOMusic_WE_POINTER_SESSION_ID', 'Process')
...
public static class MOMusicWeParallaxPointerRelay {
...
'@
Add-Type -TypeDefinition $source -Language CSharp
[MOMusicWeParallaxPointerRelay]::Run($sourceId, $expectedTitle, $expectedExecutable, $hostWindowId, $hostExecutable, $sessionId)
`.trim();
  return Buffer.from(source, 'utf16le').toString('base64');
}
...
function nativeDwmThumbnailSurfaceScript() {
  const source = String.raw`
$ErrorActionPreference = 'Stop'
$sourceId = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_SOURCE_ID', 'Process')
$expectedTitle = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_SOURCE_TITLE', 'Process')
$expectedExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_SOURCE_EXECUTABLE', 'Process')
$hostWindowId = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_HOST_WINDOW_ID', 'Process')
$hostExecutable = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_HOST_EXECUTABLE', 'Process')
$hostCornerRadius = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_HOST_CORNER_RADIUS', 'Process')
$desktopIconLayering = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_DESKTOP_ICON_LAYERING', 'Process')
$sessionId = [Environment]::GetEnvironmentVariable('MOMusic_WE_DWM_SESSION_ID', 'Process')
if ([string]::IsNullOrWhiteSpace($sourceId) -or [string]::IsNullOrWhiteSpace($expectedTitle) -or
    [string]::IsNullOrWhiteSpace($expectedExecutable) -or [string]::IsNullOrWhiteSpace($hostWindowId) -or
    [string]::IsNullOrWhiteSpace($hostExecutable) -or [string]::IsNullOrWhiteSpace($sessionId)) {
...
public sealed class MOMusicWeDwmSurfaceHost : Form {
  [StructLayout(LayoutKind.Sequential)]
  struct RECT { public int Left; public int Top; public int Right; public int Bottom; }
  ...
  MOMusicWeDwmSurfaceHost(IntPtr host, IntPtr source, string expectedTitle, int cornerRadius,
  ...
  void FollowHost() {
    ...
    // between MOMusic and the one base DWM surface. Normal top-level windows
  ...
  public static void Run(string sourceId, string expectedTitle, string expectedExecutable,
    ...
    Application.Run(new MOMusicWeDwmSurfaceHost(host, source, expectedTitle, cornerRadius,
...
try {
  [MOMusicWeDwmSurfaceHost]::Run($sourceId, $expectedTitle, $expectedExecutable, $hostWindowId, $hostExecutable, $hostCornerRadius, $desktopIconLayering)
} catch {
  ...
  return new Promise((resolve) => {
    childProcess.execFile(powerShellExecutable || 'powershell.exe', [
      ...
      env: {
        ...
        TEMP: nativeTempPath,
        TMP: nativeTempPath,
        MOMusic_NATIVE_TEMP_DIR: nativeTempPath,
        MOMusic_WE_ENGINE_TARGET: String(expectedExecutable || ''),
      },
    }, (error, stdout) => {
      if (error) {
...
class WallpaperEngineRuntime {
  constructor(options = {}) {
    ...
    this.nativeTempPath = path.resolve(String(
      ...
      || process.env.MOMusic_NATIVE_TEMP_DIR
      || path.join(process.env.LOCALAPPDATA || process.env.APPDATA || process.cwd(), 'MOMusic', 'native-helper-temp')
    ));
  ...
  _powerShellEnv(extra = {}) {
    ...
    return {
      ...
      MOMusic_NATIVE_TEMP_DIR: this.nativeTempPath,
      ...extra,
    };
  }
      ...
      try {
        ...
        child = this.dwmSurfaceSpawn(this.powerShellExecutable, [
          ...
          env: this._powerShellEnv({
            MOMusic_WE_DWM_SOURCE_ID: sourceId,
            MOMusic_WE_DWM_SOURCE_TITLE: session.locationTitle,
            MOMusic_WE_DWM_SOURCE_EXECUTABLE: session.executable,
            MOMusic_WE_DWM_HOST_WINDOW_ID: hostWindowId,
            MOMusic_WE_DWM_HOST_EXECUTABLE: hostExecutable,
            MOMusic_WE_DWM_HOST_CORNER_RADIUS: String(session.dwmSurfaceHostCornerRadius || 0),
            MOMusic_WE_DWM_DESKTOP_ICON_LAYERING: session.dwmSurfaceDesktopIconLayering === true ? '1' : '0',
            MOMusic_WE_DWM_SESSION_ID: session.sessionId,
          }),
        });
      } catch (_) {
        return false;
    ...
    const operation = (async () => {
      ...
      try {
        child = this.pointerRelaySpawn(this.powerShellExecutable, [
          ...
          stdio: ['pipe', 'pipe', 'pipe'],
          env: this._powerShellEnv({
            MOMusic_WE_POINTER_SOURCE_ID: sourceId,
            MOMusic_WE_POINTER_SOURCE_TITLE: session.locationTitle,
            MOMusic_WE_POINTER_SOURCE_EXECUTABLE: session.executable,
            MOMusic_WE_POINTER_HOST_WINDOW_ID: hostWindowId,
            MOMusic_WE_POINTER_HOST_EXECUTABLE: hostExecutable,
            MOMusic_WE_POINTER_SESSION_ID: session.sessionId,
          }),
        });
      } catch (_) {
        return false;
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.nativeExecFile(this.powerShellExecutable, [
          ...
          shell: false,
          env: this._powerShellEnv({
            MOMusic_WE_WINDOW_ACTION: String(action || ''),
            MOMusic_WE_WINDOW_SOURCE_ID: sourceId,
            MOMusic_WE_WINDOW_TITLE: String(details.locationTitle || ''),
            MOMusic_WE_WINDOW_EXECUTABLE: String(details.executable || ''),
            MOMusic_WE_HOST_WINDOW_ID: String(details.hostWindowId || ''),
            MOMusic_WE_HOST_EXECUTABLE: String(details.hostExecutable || ''),
            MOMusic_WE_HOST_CORNER_RADIUS: String(Math.max(0, Math.min(512, Number(details.cornerRadius) || 0))),
          }),
        }, finish);
      } catch (error) {
        finish(error);
  ...
  async _prepareSilentLaunchFile(session, projectFile, scenePackage) {
    ...
    const preferredStageRoot = nativeVolume === packageVolume
      ...
      : path.resolve(path.parse(scenePackage).root, 'MOMusicCache', 'wallpaper-engine-scene-stage');
    let stageRoot = preferredStageRoot;
    try {
      await fs.promises.mkdir(stageRoot, { recursive: true });
    } catch (_) {
      stageRoot = path.resolve(path.dirname(scenePackage), '.MOMusic-scene-stage');
      await fs.promises.mkdir(stageRoot, { recursive: true });
    }
  ...
  async confirmCaptureReady(expectedSessionId = '') {
    ...
    // Keep the native WE source physically aligned behind MOMusic so the
  ...
  async _verifyExecutableSignature(executable, stat) {
    ...
    try {
      const output = await this._execFileText(this.powerShellExecutable, [
        ...
        shell: false,
        env: this._powerShellEnv({
          MOMusic_WE_SIGNATURE_TARGET: executable,
        }),
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.controlExecFile(this.powerShellExecutable, [
          ...
          shell: false,
          env: this._powerShellEnv({
            MOMusic_WE_CONTROL_TARGET: executable,
            MOMusic_WE_CONTROL_COMMAND_LINE: commandLine,
            MOMusic_WE_CONTROL_WAIT: options.waitForExit === true ? '1' : '0',
            MOMusic_WE_CONTROL_WAIT_TIMEOUT: options.waitForExit === true ? '10000' : '0',
          }),
        }, finish);
      } catch (error) {
        finish(error);
  ...
  async start(id, options = {}) {
    ...
    const session = {
      ...
      sessionId,
      locationTitle: `MOMusic Wallpaper ${sessionId}`,
      sourceId: '',
      windowSourceId: '',
