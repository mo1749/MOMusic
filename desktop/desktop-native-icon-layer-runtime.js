...
function nativeIconLayerGuardCSharpSource() {
  return String.raw`
public static class MOMusicDesktopNativeIconLayerGuard {
  ...
  private static string LayoutKey(MOMusicDesktopIconShapeNative.ProbeResult result) {
    System.Text.StringBuilder key = new System.Text.StringBuilder();
    ...
    if (result.icons != null) {
      foreach (MOMusicDesktopIconShapeNative.IconRect icon in result.icons) {
  ...
  private static void Emit(MOMusicDesktopIconShapeNative.ProbeResult result, bool force) {
    ...
    if (result.icons != null) {
      for (int index = 0; index < result.icons.Length; index++) {
        ...
        MOMusicDesktopIconShapeNative.IconRect icon = result.icons[index];
  ...
  private static void ApplyAndEmit(bool force) {
    ...
    MOMusicDesktopIconShapeNative.ProbeResult result = MOMusicDesktopIconShapeNative.Probe();
  ...
  public static void Run(int debounceMs, int rebindMs, int ownerProcessId,
    ...
    inputThread.Name = "MOMusic native desktop icon layer input";
    inputThread.Start();
...
[MOMusicDesktopNativeIconLayerGuard]::Run(${debounceMs}, ${rebindMs}, ${ownerProcessId}, [Int64]${iconHostWindowId}, [Int64]${listViewWindowId}, [Int64]${mainWindowId}, ${bounds.x}, ${bounds.y}, ${bounds.width}, ${bounds.height}, ${inputExpression}, ${outputExpression})
`;
}

function nativeIconLayerNamedPipeScript(options = {}) {
  const rawBody = nativeIconLayerGuardScript({ ...options, namedPipeIo: true });
  const invocationMarker = '\n[MOMusicDesktopNativeIconLayerGuard]::Run(';
...
function externalGuardTransport(options = {}) {
  ...
  const pipeBase = `MOMusicNativeIconLayer-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
...
function startNativeDesktopIconLayer(options = {}) {
  ...
  const scriptPath = path.join(
    scriptDirectory,
    `MOMusic-native-icon-layer-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`
  );
