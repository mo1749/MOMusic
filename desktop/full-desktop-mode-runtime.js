...
if (-not ("MOMusicFullDesktopNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class MOMusicFullDesktopNative {
...
try {
  try { $previousDpiContext = [MOMusicFullDesktopNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $target = [IntPtr]::new([Int64]${hwnd})
  if (-not [MOMusicFullDesktopNative]::IsWindow($target)) { throw "FULL_DESKTOP_TARGET_NOT_FOUND" }
  [MOMusicFullDesktopNative]::SetParent($target, [IntPtr]::Zero) | Out-Null
  $GWL_STYLE = -16
  $WS_POPUP = [Int64]0x80000000
  $WS_CHILD = [Int64]0x40000000
  $style = [MOMusicFullDesktopNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  $topLevelStyle = ($style -band (-bnot $WS_CHILD)) -bor $WS_POPUP
  [MOMusicFullDesktopNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$topLevelStyle)) | Out-Null
  $verifiedStyle = [MOMusicFullDesktopNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  ...
  $parent = [MOMusicFullDesktopNative]::GetParent($target)
  if ($parent -ne [IntPtr]::Zero) { throw "FULL_DESKTOP_DETACH_FAILED" }
  if (-not [MOMusicFullDesktopNative]::SetWindowPos($target, [IntPtr]::Zero, ${x}, ${y}, ${width}, ${height}, 0x0030)) { throw "FULL_DESKTOP_POSITION_FAILED" }
  $rect = New-Object MOMusicFullDesktopNative+RECT
  if (-not [MOMusicFullDesktopNative]::GetWindowRect($target, [ref]$rect)) { throw "FULL_DESKTOP_BOUNDS_ACK_FAILED" }
  ...
  if ($previousDpiContext -ne [IntPtr]::Zero) {
    try { [MOMusicFullDesktopNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
  }
}
`;
}
...
function desktopWindowCoexistAttachScript(input = {}) {
  ...
  return `
$ErrorActionPreference = "Stop"
if (-not ("MOMusicDesktopCoexistNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
using System.Text;
public static class MOMusicDesktopCoexistNative {
...
$previousDpiContext = [IntPtr]::Zero
try {
  try { $previousDpiContext = [MOMusicDesktopCoexistNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $target = [IntPtr]::new([Int64]${hwnd})
  if (-not [MOMusicDesktopCoexistNative]::IsWindow($target)) { throw "FULL_DESKTOP_TARGET_NOT_FOUND" }
  $iconHost = [IntPtr]::Zero
  $defView = [IntPtr]::Zero
  $listView = [IntPtr]::Zero
  $hostClass = ""
  if (-not [MOMusicDesktopCoexistNative]::FindDesktopIconHost([ref]$iconHost, [ref]$defView, [ref]$listView, [ref]$hostClass)) {
    throw "FULL_DESKTOP_ICON_HOST_NOT_FOUND"
  }
  $parentRect = New-Object MOMusicDesktopCoexistNative+RECT
  if (-not [MOMusicDesktopCoexistNative]::GetWindowRect($defView, [ref]$parentRect)) { throw "FULL_DESKTOP_ICON_HOST_BOUNDS_FAILED" }
  $GWL_STYLE = -16
  $WS_POPUP = [Int64]0x80000000
  $WS_CHILD = [Int64]0x40000000
  $style = [MOMusicDesktopCoexistNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  $childStyle = ($style -band (-bnot $WS_POPUP)) -bor $WS_CHILD
  [MOMusicDesktopCoexistNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$childStyle)) | Out-Null
  [MOMusicDesktopCoexistNative]::SetParent($target, $defView) | Out-Null
  $verifiedStyle = [MOMusicDesktopCoexistNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  if (($verifiedStyle -band $WS_CHILD) -eq 0 -or ($verifiedStyle -band $WS_POPUP) -ne 0) { throw "FULL_DESKTOP_ICON_HOST_STYLE_FAILED" }
  if ([MOMusicDesktopCoexistNative]::GetParent($target) -ne $defView) { throw "FULL_DESKTOP_ICON_HOST_ATTACH_FAILED" }
  $localX = ${x} - $parentRect.Left
  $localY = ${y} - $parentRect.Top
  # Keep the one complete MOMusic surface below Explorer's real SysListView32.
  # The native icon layer makes Explorer's black background transparent while
  # retaining the real icon pixels and their native hit testing above MOMusic.
  if (-not [MOMusicDesktopCoexistNative]::SetWindowPos($target, [IntPtr]::new(1), $localX, $localY, ${width}, ${height}, 0x0030)) {
    throw "FULL_DESKTOP_ICON_HOST_POSITION_FAILED"
  }
  $rect = New-Object MOMusicDesktopCoexistNative+RECT
  if (-not [MOMusicDesktopCoexistNative]::GetWindowRect($target, [ref]$rect)) { throw "FULL_DESKTOP_ICON_HOST_BOUNDS_ACK_FAILED" }
  $actualWidth = $rect.Right - $rect.Left
  $actualHeight = $rect.Bottom - $rect.Top
  ...
  if ($previousDpiContext -ne [IntPtr]::Zero) {
    try { [MOMusicDesktopCoexistNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
  }
}
`;
}
...
class FullDesktopModeRuntime {
    ...
    watcher = this.startDesktopIconWatcherImpl({
      ...
      onExit: (details = {}) => {
        ...
        if (shouldRebind) {
          ...
          // MOMusic HWND. Explorer has confirmed restoration, so keep the
  ...
  applyInteractivePointerRoute(win = this.window, options = {}) {
    ...
    // A locked desktop surface passes normal MOMusic areas through to
  ...
  prepareWindow(win, bounds, options = {}) {
    ...
    // The visual HWND remains one continuous MOMusic surface below Explorer's
  ...
  async reconcileInteractiveInternal(reason = 'interactive-reconcile') {
    ...
    try {
      ...
      // WE base alive behind enabled=true plus a hidden MOMusic HWND.
