...
function desktopIconProbeScript(options = {}) {
  ...
  const invocation = options.invoke === false
    ? ''
    : '[MOMusicDesktopIconShapeNative]::Probe() | ConvertTo-Json -Compress -Depth 5';
  return `
$ErrorActionPreference = "Stop"
if (-not ("MOMusicDesktopIconShapeNative" -as [type])) {
Add-Type @"
using System;
using System.Collections.Generic;
using System.Runtime.InteropServices;

public static class MOMusicDesktopIconShapeNative {
...
function desktopIconWatcherCSharpSource() {
  return `
public static class MOMusicDesktopIconShapeWatcherNative {
  ...
  private static bool FindTarget(out IntPtr topLevelHost, out IntPtr iconHost, out IntPtr listView,
    ...
    MOMusicDesktopIconShapeNative.FindDesktopListView(out topLevelHost, out iconHost, out listView);
    processId = 0;
    threadId = 0;
  ...
  private static void RebindIfNeeded(bool forceSnapshot) {
    ...
    if (changed || forceSnapshot) {
      try { Emit(MOMusicDesktopIconShapeNative.Probe(), changed || forceSnapshot); }
  ...
  private static void EmitPendingLayout() {
    ...
    if (!ready) return;
    try { Emit(MOMusicDesktopIconShapeNative.Probe(), false); }
    catch { EmitError("DESKTOP_ICON_PROBE_FAILED"); }
  }
  ...
  public static void Run(int debounceMs, int rebindMs) {
    ...
    inputThread.Name = "MOMusic desktop icon watcher input";
    inputThread.Start();
...
[MOMusicDesktopIconShapeWatcherNative]::Run(${debounceMs}, ${rebindMs})
`;
}
