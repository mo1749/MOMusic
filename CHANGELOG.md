...
- P0 installer safety fix: installation now defaults to the first available non-C drive from `D:\MOMusic` through `Z:\MOMusic`; it falls back to `C:\MOMusic` only when no D-Z drive exists.
- The installer now forces the target path into a dedicated `MOMusic` folder, blocks non-empty non-MOMusic-owned targets, and blocks C drive installs when a D-Z drive is available.
- Existing registered installs in a dedicated `...\MOMusic` folder are adopted in place so users can run the new setup over their current installation and receive the safe uninstaller.
- Existing dedicated `...\MOMusic` folders that already contain MOMusic files can be overwritten even if they were created before the new safety marker existed.
- The new uninstaller no longer removes the whole installation root or app subfolders recursively; it deletes only known top-level MOMusic/Electron files and leaves unrelated files in place.
- Legacy uninstallers without the new safety marker are skipped during install; the setup deletes only the old `Uninstall MOMusic.exe` file and registry entry so it cannot indirectly trigger an older unsafe uninstall routine.
...
- 安装包支持用户自由选择安装目录；默认仍优先 `D:\MOMusic`，选择盘根目录时会自动补全 `MOMusic` 文件夹。
...
- 发布 MOMusic 首个正式版本，GitHub 主页和安装包版本统一为 `1.0.0`。
