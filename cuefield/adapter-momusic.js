...
const MOMusic_BEAT_COMBOS = ['', 'downbeat', 'push', 'drop', 'rebound', 'accent'];
...
function normalizeBeatEvent(raw, index, gridStep) {
  if (Array.isArray(raw)) {
    ...
    const combo = MOMusic_BEAT_COMBOS[Math.max(0, Math.round(toNumber(raw[7], 0)))] || '';
    return {
      time,
      index,
...
function normalizeMOMusicBeatMap(track, map, extra = {}) {
