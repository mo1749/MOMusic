...
const LOGIN_EASTER_EGG_GATE_FILE = String(process.env.MOMusic_LOGIN_EASTER_EGG_GATE_FILE || '');
const LOGIN_EASTER_EGG_GATE_VERSION = String(
  process.env.MOMusic_LOGIN_EASTER_EGG_GATE_VERSION ||
...
function readUpdateConfig(pkg) {
  const local = (pkg && (pkg.momusic || pkg.MOMusic) && ((pkg.momusic || {}).update || (pkg.MOMusic || {}).update)) || {};
  ...
  const repoHint = process.env.MOMusic_UPDATE_REPOSITORY
  ...
  const owner = process.env.MOMusic_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.MOMusic_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.MOMusic_UPDATE_MANIFEST
      ...
      || process.env.MOMusic_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.MOMusic_UPDATE_MIRRORS || process.env.MOMusic_UPDATE_MIRROR || '';
...
function pickReleaseAsset(assets) {
  ...
  // 必须优先选 NSIS 安装包（Setup）：Portable 便携版不是安装器，静默安装 /S 对它无效，
  // 且资产列表中 Portable 常排在前面，直接 find 第一个 exe 会选错
  const preferred = list.find(a => /setup[^/\\]*\.(exe|msi)$/i.test(a && a.name || ''))
    || list.find(a => /\.(exe|msi)$/i.test(a && a.name || '') && !/portable/i.test(a && a.name || ''))
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `MOMusic-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `MOMusic-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('MOMusic v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `MOMusic/${APP_VERSION}` },
    });
...
// 更新下载守护：连接阶段与响应体读取阶段分别计时。
// fetchWithTimeout 只保护到响应头返回，读取大文件途中若上游停滞，
// 之前要等 undici 默认 5 分钟 bodyTimeout 才报错切线路，这里 30 秒内无新数据即中断。
function createUpdateDownloadGuard(connectTimeoutMs, stallTimeoutMs) {
  const controller = new AbortController();
  const state = { timer: null, reason: null };
  function clearTimer() {
    if (state.timer) { clearTimeout(state.timer); state.timer = null; }
  }
  function armWith(code, reason, timeoutMs) {
    clearTimer();
    state.reason = updateError(code, reason);
    state.timer = setTimeout(() => controller.abort(), timeoutMs);
  }
  return {
    signal: controller.signal,
    armConnect() { armWith('UPDATE_CONNECT_TIMEOUT', '连接服务器超时', connectTimeoutMs || 14000); },
    armStall() { armWith('UPDATE_STALL_TIMEOUT', '下载中途停滞', stallTimeoutMs || 30000); },
    wrapError(err) {
      if (err && (err.name === 'AbortError' || err.code === 'ABORT_ERR') && state.reason) return state.reason;
      return err;
    },
    dispose: clearTimer,
  };
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `MOMusic-${latestVersion}-Setup.exe`;
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      signal: controller.signal,
      headers: {
        'User-Agent': `MOMusic/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('MOMusic v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `MOMusic-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `MOMusic-${version || APP_VERSION}.exe`;
}
...
async function downloadUpdateAssetWithMirrors(job) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      const guard = createUpdateDownloadGuard(14000, 30000);
      let resp;
      try {
        guard.armConnect();
        ...
        throw guard.wrapError(fetchErr);
      } finally {
        guard.dispose();
      }
      if (!resp.ok) throw updateError('HTTP_' + resp.status, 'HTTP ' + resp.status);
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.MOMusic-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'MOMusic-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
async function downloadPatchBufferFromCandidate(job, candidate, index, total) {
  ...
  const guard = createUpdateDownloadGuard(12000, 30000);
  let resp;
  try {
    guard.armConnect();
    resp = await fetch(candidate.url, {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'MOMusic',
      productName: APP_PACKAGE.productName || 'MOMusic',
      version: APP_VERSION,
      update: {
  ...
  if (pn === '/api/kugou/recommendations') {
    try {
      const limit = Math.max(4, Math.min(100, parseInt(url.searchParams.get('limit') || '100', 10) || 100));
  ...
  if (pn === '/api/qishui/logout') {
    try {
      try { await qishuiQrLogin.clear(); } catch (clearErr) { console.warn('[QishuiLogout] v6 clear failed:', clearErr.message); }
      saveQishuiCookie('');
  ...
  if (pn === '/api/cover') {
    try {
      ...
      // 根据 URL 域名选择正确的 Referer，防止 CDN 反盗链
      const coverHost = (coverUrl.match(/^https?:\/\/([^\/]+)/) || [])[1] || '';
      const coverReferer = /qlogo\.cn/i.test(coverHost) ? 'https://y.qq.com/'
