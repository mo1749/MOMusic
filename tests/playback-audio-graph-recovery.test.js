...
function makeHarness(options = {}) {
  ...
  const audio = makeAudio({
    ...
    __MOMusicQueueItemKey: 'queue-alpha',
  });
...
function testCaptureTrackSwitchCreatesFreshAudioLifetime() {
  ...
  assert.equal(context.source.__MOMusicUsesCapture, false);
  assert.equal(audio.__MOMusicMediaSourceBound, true);
  ...
  context.source.__MOMusicUsesCapture = true;
  context.audioSourceMedia = audio;
  context.audioReady = true;
  assert.equal(context.source.__MOMusicUsesCapture, true, 'test did not establish the legacy capture-backed state');
  ...
  assert.equal(audio.__MOMusicMediaSourceBound, true, 'lifetime MediaElementSource marker was cleared');
  ...
  assert.equal(freshAudio.__MOMusicQueueItemKey, 'queue-alpha', 'fresh Audio lost queue identity');
  ...
  assert.equal(context.initAudio(), true);
  assert.equal(context.source.__MOMusicUsesCapture, false, 'fresh Audio did not establish MediaElementSource');
  ...
  assert.equal(freshAudio.__MOMusicMediaSourceBound, true);
  assert.equal(freshAudio.__MOMusicQueueItemKey, 'queue-alpha');
}
...
function testTwoAdvancingSilentSamplesTriggerCaptureRebuild() {
  ...
  assert.equal(context.source.__MOMusicUsesCapture, false, 'bound media was incorrectly moved to captureStream');
...
function testTransientCaptureSourceFailureRetainsForcedCaptureRetry() {
  ...
  const originalAudio = audio;
  audio.__MOMusicForceCaptureSource = true;
  ...
  assert.equal(audio.__MOMusicForceCaptureSource, true, 'failed capture attempt consumed the forced-capture retry');
  ...
  assert.equal(context.source.__MOMusicUsesCapture, true);
  ...
  assert.strictEqual(context.audio, originalAudio);
  assert.equal(!!audio.__MOMusicMediaSourceBound, false);
  assert.equal(audio.__MOMusicQueueItemKey, 'queue-alpha');
}
...
function testForcedCaptureWaitsForMediaReadyState() {
  ...
  assert.equal(audio.__MOMusicForceCaptureSource, true, 'ready-state wait consumed the forced-capture marker');
  assert.strictEqual(context.audio, originalAudio);
