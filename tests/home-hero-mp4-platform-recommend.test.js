...
test('home hero video has isolated persistence and never enters global wallpaper paths', () => {
  assert.match(dashboardScript, /HOME_DASHBOARD_VIDEO_DB_NAME\s*=\s*['"]MOMusic-home-dashboard-video-v1['"]/);
  assert.match(dashboardScript, /HOME_DASHBOARD_VIDEO_BLOB_ID\s*=\s*['"]home-hero-video['"]/);
  assert.match(dashboardScript, /HOME_DASHBOARD_VIDEO_META_KEY\s*=\s*['"]MOMusic-home-dashboard-video-meta-v1['"]/);
