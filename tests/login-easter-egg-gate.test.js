...
async function run() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'MOMusic-login-gate-'));
  try {
    ...
    assert(main.includes("ipcMain.handle('MOMusic-login-easter-egg-reset'"));
    ...
    assert(preload.includes("resetLoginEasterEgg: () => ipcRenderer.invoke('MOMusic-login-easter-egg-reset')"));
