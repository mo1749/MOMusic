...
function testStaticRecoveryWiring() {
  ...
  assert(/audio\.__MOMusicTrackSwitchToken = token/.test(startText));
  assert(/audioEl !== audio/.test(progressText));
  assert(/__MOMusicTrackSwitchToken\) !== Number\(trackSwitchToken\)/.test(progressText));
  assert(/playbackMediaMatchesCurrentQueueItem\(audioEl\)/.test(progressText));
  assert(/ownerQueueItemKey:\s*String\(audioEl\.__MOMusicQueueItemKey/.test(progressText));
