...
test('legacy search histories migrate into one list shared by every tab', () => {
  const values = new Map([
    ['MOMusic-search-history', JSON.stringify(['晴天', '晴天', '夜曲'])],
  ]);
  const sandbox = {
    searchMode: 'song',
    localStorage: {
  ...
  stored = JSON.parse(values.get('MOMusic-search-history'));
  assert.deepEqual(stored.items, ['一路向北', '晴天', '夜曲', '海阔天空', '稻香', '故事 FM']);

  sandbox.writeSearchHistory([], 'qq');
  assert.deepEqual(JSON.parse(values.get('MOMusic-search-history')).items, []);
