...
test('enable defaults to a fully interactive MOMusic desktop', async () => {
  ...
  assert.equal(calls.coexist.length, 1, 'interactive enable must attach the complete MOMusic surface below the real desktop icons');
  ...
  assert.deepEqual(win.shape, [], 'continuous MOMusic surface must not contain desktop icon holes');
});
...
test('passive preparation failure leaves the full MOMusic window interactive', async () => {
  const win = new FakeBrowserWindow();
  const { runtime, calls } = makeRuntime({
...
test('software lock passes normal areas through while the controller remains an unlock island', async () => {
  ...
  assert.equal(win.ignoreMouse, false, 'unlock from the controller must immediately restore natural MOMusic input');
...
test('coexist attach places the complete MOMusic surface below SysListView without mouse synthesis', () => {
...
test('unexpected watcher exit with confirmed restoration queues recovery without hiding MOMusic', async () => {
  const win = new FakeBrowserWindow();
