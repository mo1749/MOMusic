...
function consumeFrameGate(gate, now, dt, fps, force, reason) {
  ...
  if (gate.lastRunAt && now - gate.lastRunAt < minGap) {
    gate.skips += 1;
    if (window.__MOMusicPerf && window.__MOMusicPerf.count) {
      window.__MOMusicPerf.count('frameGate.' + gate.name + '.skipped');
    }
    return 0;
  }
  return runFrameGate(gate, now, dt);
...
function runFrameGate(gate, now, fallbackDt) {
  ...
  gate.runs += 1;
  if (window.__MOMusicPerf && window.__MOMusicPerf.count) {
    window.__MOMusicPerf.count('frameGate.' + gate.name + '.runs');
  }
  return gate.lastDt;
}
