function markAppPerf(name) {
  try {
    ...
    if (performance && performance.mark) performance.mark('MOMusic:' + name);
    if (appPerfMarks.length <= 16) console.debug('[MOMusicPerf]', name, Math.round(value) + 'ms');
  } catch (e) { }
}
  ...
  try {
    ...
    var observer = new PerformanceObserver(function (list) {
      list.getEntries().forEach(function (entry) {
        ...
        console.debug('[MOMusicPerf] longtask', Math.round(entry.startTime) + 'ms', Math.round(entry.duration) + 'ms');
      });
    });
