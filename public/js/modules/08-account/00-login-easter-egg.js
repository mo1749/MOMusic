var LOGIN_EASTER_EGG_BROWSER_PREVIEW_KEY = 'MOMusic-login-easter-egg-browser-preview-v1';
...
function bindLoginEasterEggGate() {
  ...
  input.addEventListener('compositionend', function (event) {
    ...
    // 延迟处理确保 input.value 已更新（中文输入法兼容）
...
async function requestLoginEasterEggUnlock(value) {
  ...
  if (value !== previewPassword && !/^sjhp$/i.test(value)) return { ok: false, unlocked: false, error: 'LOGIN_EASTER_EGG_INVALID' };
...
function skipLoginEasterEgg() {
  clearLoginEasterEggFocusRetries();
  requestLoginEasterEggUnlock(loginEasterEggAnswer()).then(function (result) {
    if (!result || !result.ok) {
      loginEasterEggState.ready = true;
      loginEasterEggState.unlocked = true;
      try { localStorage.setItem(LOGIN_EASTER_EGG_BROWSER_PREVIEW_KEY, '1'); } catch (_) {}
    ...
    ensureLoginEasterEggStatus(true).then(function () {
      completeLoginEasterEggUnlock();
    });
  }).catch(function () {
    // IPC 调用失败时回退到本地状态
