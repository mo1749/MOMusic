...
var ACCOUNT_PROVIDER_KEYS = ['netease', 'qq', 'kugou', 'kugou-concept', 'qishui', 'spotify'];
var ACCOUNT_PROVIDER_ORDER_STORE_KEY = 'MOMusic-account-provider-order-v1';
var ACCOUNT_PROVIDER_VISIBLE_STORE_KEY = 'MOMusic-account-provider-visible-v1';
...
function platformMeta(provider) {
  ...
  if (provider === 'kugou-concept') return { key: 'kugou-concept', short: 'KGC', label: '酷狗概念版', app: '酷狗概念版 App', dot: 'kugou-concept' };
...
function platformStatus(provider) {
  ...
  if (provider === 'kugou-concept') return kugouConceptLoginStatus;
...
function providerVipLevel(provider, status) {
  ...
  if (raw === 'none' || raw === 'free' || raw === 'open' || raw === 'unknown' ||
      raw === 'no vip' || raw === 'no_vip' || raw === 'no-vip' || raw === 'normal' || raw === '无vip') {
    var vip = providerVipType(provider, status);
    if (vip > 0 || providerFlagEnabled(status, ['isVip', 'is_vip', 'vip', 'vipFlag', 'vipflag'])) return 'vip';
    return 'none';
  }
  var vip = providerVipType(provider, status);
...
function hasAnyPlatformLogin() {
  return hasPlatformLogin('netease') || hasPlatformLogin('qq') || hasPlatformLogin('kugou') || hasPlatformLogin('kugou-concept') || hasPlatformLogin('qishui') || hasPlatformLogin('spotify');
}
...
function providerAvatarSrc(provider, status) {
  ...
  var fill = provider === 'qq' ? '#bfd66b' : (provider === 'kugou' ? '#56e0ff' : (provider === 'kugou-concept' ? '#9d8bff' : (provider === 'qishui' ? '#45d68f' : (provider === 'spotify' ? '#1ed760' : '#d95b67'))));
  var bg = provider === 'qq' ? '#11150b' : (provider === 'kugou' ? '#071722' : (provider === 'kugou-concept' ? '#0d0a1f' : (provider === 'qishui' ? '#071a12' : (provider === 'spotify' ? '#06140a' : '#180b0f'))));
