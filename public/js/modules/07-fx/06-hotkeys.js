...
function ensureHotkeyModal() {
  ...
  modal.innerHTML =
    ...
    '<div><div class="hotkey-title">热键设置</div><div class="hotkey-sub">局内热键只在 MOMusic 窗口内生效；全局热键会向系统注册，并检测是否被占用。</div></div>' +
...
function hotkeyStatusMarkup(scope, actionKey, binding, duplicate) {
  ...
  if (duplicate && duplicate[binding] > 1) return '<span class="hotkey-status conflict"><span class="source-icon">!</span>MOMusic 内部重复</span>';
