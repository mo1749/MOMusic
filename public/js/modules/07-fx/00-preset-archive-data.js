...
var USER_FX_ARCHIVE_STORE_KEY = 'MOMusic-user-fx-archives-v1';
var USER_FX_ARCHIVE_EXPORT_TYPE = 'MOMusic-user-fx-archive';
...
function normalizeFxArchiveSnapshot(raw) {
  ...
  return {
    ...
    galaxyArms: Math.round(archiveNumber(raw, 'galaxyArms', fxDefaults.galaxyArms, 2, 8)),
    galaxyTwist: archiveNumber(raw, 'galaxyTwist', fxDefaults.galaxyTwist, 0, 2),
...
function safeArchiveFileName(name) {
  return String(name || 'MOMusic 用户存档').replace(/[\\/:*?"<>|]+/g, '-').slice(0, 48) + '.json';
}
