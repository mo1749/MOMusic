var WALLPAPER_ENGINE_SELECTION_STORE_KEY = 'MOMusic-wallpaper-engine-selection-v1';
var WALLPAPER_ENGINE_HIDDEN_STORE_KEY = 'MOMusic-wallpaper-engine-hidden-v1';
var WALLPAPER_ENGINE_FAVORITE_STORE_KEY = 'MOMusic-wallpaper-engine-favorites-v1';
...
function wallpaperEnginePointerActivityReady() {
  ...
  // after that source is parked even though the real MOMusic window is
...
async function hardenWallpaperEngineCaptureStream(stream) {
  ...
  try {
    stream.__MOMusicCursorSuppressed = !!tracks.length && cursorResults.every(function (value) { return value === true; });
  } catch (e4) { }
  return stream;
}
...
window.__MOMusicSyncWallpaperEngineCaptureFrameRate = syncWallpaperEngineCaptureFrameRate;
...
async function openWallpaperEngineCaptureStream(sessionId, fps, sourceId, options) {
  ...
  try {
    if (diagnostics.purpose === 'dwm-glass') window.__MOMusicWallpaperEngineGlassCaptureDiagnostics = diagnostics;
    else window.__MOMusicWallpaperEngineCaptureDiagnostics = diagnostics;
  } catch (e) { }
  function recordAttempt(path, stream, error) {
    ...
    var entry = {
      path: path,
      ok: !!track,
      cursorSuppressed: !!(stream && stream.__MOMusicCursorSuppressed),
      settings: null,
      constraints: null,
  ...
  if (/^window:\d+:\d+$/.test(sourceId) && typeof navigator.mediaDevices.getUserMedia === 'function') {
    try {
      ...
      if (sourceStream && sourceStream.getVideoTracks && sourceStream.getVideoTracks().length) {
        ...
        try { hardenedSourceStream.__MOMusicCapturePath = 'source-id-media'; } catch (e4) { }
        ...
        if (sourceAttempt.cursorSuppressed || options.trustedCursorFreeSurface === true
          || window.__MOMusicAllowUnverifiedSourceCapture === true) {
          try { hardenedSourceStream.__MOMusicUnverifiedCursorCapture = !sourceAttempt.cursorSuppressed; } catch (e5) { }
  ...
  if (options.sourceIdOnly !== true && typeof navigator.mediaDevices.getDisplayMedia === 'function') {
    try {
      ...
      if (displayStream && displayStream.getVideoTracks && displayStream.getVideoTracks().length) {
        ...
        try { hardenedDisplayStream.__MOMusicCapturePath = 'display-media'; } catch (e6) { }
...
window.__MOMusicPrepareWallpaperEngineCapture = async function (sessionId, fps, sourceId) {
  sessionId = String(sessionId || '');
...
window.__MOMusicPrepareWallpaperEngineGlassCapture = async function (sessionId, fps, sourceId) {
  sessionId = String(sessionId || '');
  if (!/^[a-f0-9]{24}$/i.test(sessionId)) return { ok: false, error: 'WALLPAPER_ENGINE_SESSION_INVALID' };
  try {
    var stream = await openWallpaperEngineCaptureStream(sessionId, fps, sourceId, {
...
window.__MOMusicPrepareWallpaperEngineHostBoundsChange = function (sessionId, reason) {
  sessionId = String(sessionId || '');
...
window.__MOMusicPrepareWallpaperEngineDesktopPreview = function (sessionId, reason) {
  sessionId = String(sessionId || '');
...
async function ensureWallpaperEngineGlassSamplerCapture(sessionId, layerToken, attempt) {
  ...
  try {
    ...
    document.body.classList.add('wallpaper-engine-glass-sampler-ready');
    try {
      window.__MOMusicWallpaperEngineGlassSamplerState = {
        ok: true,
        sessionId: sessionId,
...
function wallpaperEngineRuntimeErrorText(error) {
  ...
  if (/WALLPAPER_ENGINE_HOST_ELEVATED/.test(code)) return 'MOMusic 正以管理员身份运行，无法捕获 WE 实时窗口；请取消“以管理员身份运行”后重启播放器';
  ...
  if (/WALLPAPER_ENGINE_WINDOW_CLOSE_FAILED/.test(code)) return '上一次 MOMusic 实时壁纸窗口仍在收尾，请稍后重试；Wallpaper Engine 本体会保留';
  ...
  if (/InvalidStateError/.test(code)) return 'WE 实时画面连接需要 MOMusic 保持在前台';
...
function handleWallpaperEngineHostBoundsChange(payload) {
  ...
  if (phase === 'prepare' && typeof window.__MOMusicPrepareWallpaperEngineHostBoundsChange === 'function') {
    window.__MOMusicPrepareWallpaperEngineHostBoundsChange(payload && payload.sessionId, payload && payload.reason);
  }
}
  ...
  if (!document._wallpaperEngineKeyBound) {
    ...
    document.addEventListener('visibilitychange', function () {
      ...
      if (wallpaperEngineSelection.kind === 'engine') {
        ...
        if (document.hidden) {
          window.__MOMusicPrepareWallpaperEngineHostBoundsChange(wallpaperEngineNativeSessionId, 'document-hidden');
          stopWallpaperEngineNativeSession();
