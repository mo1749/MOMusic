...
window.__MOMusicRenderPerf = renderPerfState;
if (window.__MOMusicPerf && typeof window.__MOMusicPerf.registerRenderState === 'function') {
  window.__MOMusicPerf.registerRenderState(renderPerfState);
} else {
  window.__MOMusicPerf = renderPerfState;
}
...
function shouldSkipAdaptiveRenderFrame(now) {
  ...
  if (cadence) {
    ...
    if (cadence.divisor > 1 && (renderPerfState.adaptiveCadenceTick - 1) % cadence.divisor !== 0) {
      ...
      if (window.__MOMusicPerf && window.__MOMusicPerf.count) window.__MOMusicPerf.count('frame.skipped');
      return true;
    }
...
window.__MOMusicMainFrameGates = mainFrameGates;
...
function animate() {
  ...
  var perfProbe = window.__MOMusicPerf;
  ...
  var sonicPresetActiveEarly = window.MOMusicSonicTopography && MOMusicSonicTopography.isActive(fx);
  ...
  if (window.MOMusicSonicTopography) {
    MOMusicSonicTopography.update(dt, {
      scene: scene,
      fx: fx,
  ...
  var heartPulsePerfStart = performance.now();
  if (window.MOMusicHeartPulse) {
    MOMusicHeartPulse.update(dt, {
      scene: scene,
      fx: fx,
      time: uniforms.uTime.value,
      screenHeight: window.innerHeight,
