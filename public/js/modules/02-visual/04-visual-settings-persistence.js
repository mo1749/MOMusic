...
function readSavedLyricLayout() {
  ...
  try {
    ...
    // 像素歌词独立化迁移：旧存档里 pixelKaomojiEnabled 是"像素歌词"开关，
    // 迁移后不再关闭颜文字粒子（默认开启），原开关值转给 pixelKaomojiLyrics
    ...
    return {
      ...
      goldenCoreGlow: clampRange(raw.goldenCoreGlow == null ? fxDefaults.goldenCoreGlow : Number(raw.goldenCoreGlow), 0.2, 2),
      ...
      goldenCoreOrbitCount: clampRange(Math.round(raw.goldenCoreOrbitCount == null ? fxDefaults.goldenCoreOrbitCount : Number(raw.goldenCoreOrbitCount)), 2, 10),
      goldenCoreOrbitSize: clampRange(raw.goldenCoreOrbitSize == null ? fxDefaults.goldenCoreOrbitSize : Number(raw.goldenCoreOrbitSize), 0.5, 2),
      goldenCoreDensity: clampRange(raw.goldenCoreDensity == null ? fxDefaults.goldenCoreDensity : Number(raw.goldenCoreDensity), 0.3, 2),
      goldenCoreSpin: clampRange(raw.goldenCoreSpin == null ? fxDefaults.goldenCoreSpin : Number(raw.goldenCoreSpin), 0, 2),
      goldenCoreStardust: clampRange(raw.goldenCoreStardust == null ? fxDefaults.goldenCoreStardust : Number(raw.goldenCoreStardust), 0, 1.5),
      goldenCoreBreath: clampRange(raw.goldenCoreBreath == null ? fxDefaults.goldenCoreBreath : Number(raw.goldenCoreBreath), 0, 1),
      ...
      treeCanopyGlow: clampRange(raw.treeCanopyGlow == null ? fxDefaults.treeCanopyGlow : Number(raw.treeCanopyGlow), 0.2, 2),
      treeCanopyWind: clampRange(raw.treeCanopyWind == null ? fxDefaults.treeCanopyWind : Number(raw.treeCanopyWind), 0, 2),
      treeCanopySway: clampRange(raw.treeCanopySway == null ? fxDefaults.treeCanopySway : Number(raw.treeCanopySway), 0, 2),
      treeCanopyStaffCount: clampRange(Math.round(raw.treeCanopyStaffCount == null ? fxDefaults.treeCanopyStaffCount : Number(raw.treeCanopyStaffCount)), 2, 7),
      treeCanopyNoteDensity: clampRange(raw.treeCanopyNoteDensity == null ? fxDefaults.treeCanopyNoteDensity : Number(raw.treeCanopyNoteDensity), 0.4, 2),
      treeCanopyNoteSize: clampRange(raw.treeCanopyNoteSize == null ? fxDefaults.treeCanopyNoteSize : Number(raw.treeCanopyNoteSize), 0.5, 2),
      treeCanopyBeatResponse: clampRange(raw.treeCanopyBeatResponse == null ? fxDefaults.treeCanopyBeatResponse : Number(raw.treeCanopyBeatResponse), 0, 2),
      treeCanopyAmbient: clampRange(raw.treeCanopyAmbient == null ? fxDefaults.treeCanopyAmbient : Number(raw.treeCanopyAmbient), 0, 2),
      ...
      heartPulseSpeed: clampRange(raw.heartPulseSpeed == null ? fxDefaults.heartPulseSpeed : Number(raw.heartPulseSpeed), 0.35, 2.4),
      heartPulseBeatResponse: clampRange(raw.heartPulseBeatResponse == null ? fxDefaults.heartPulseBeatResponse : Number(raw.heartPulseBeatResponse), 0, 2),
      heartPulseGlow: clampRange(raw.heartPulseGlow == null ? fxDefaults.heartPulseGlow : Number(raw.heartPulseGlow), 0.25, 2),
      heartPulseGrid: clampRange(raw.heartPulseGrid == null ? fxDefaults.heartPulseGrid : Number(raw.heartPulseGrid), 0, 1),
      heartPulseTrail: clampRange(raw.heartPulseTrail == null ? fxDefaults.heartPulseTrail : Number(raw.heartPulseTrail), 0.15, 1.5),
...
function currentFxAutosaveTouchedKeys(reason, payload) {
  ...
  var map = {
    ...
    goldenCoreColor: ['goldenCoreColor'],
    goldenCoreHaloColor: ['goldenCoreHaloColor'],
    heartPulseLineColor: ['heartPulseLineColor'],
    heartPulseGlowColor: ['heartPulseGlowColor'],
    heartPulseHeartColor: ['heartPulseHeartColor'],
...
function saveLyricLayout(opts) {
  ...
  try {
    ...
    var autosavePayload = {
      ...
      pixelKaomojiEnabled: fx.pixelKaomojiEnabled !== false,
      pixelKaomojiLyrics: fx.pixelKaomojiLyrics !== false,
      pixelLyricsEnabled: fx.pixelLyricsEnabled === true,
      ...
      goldenCoreGlow: clampRange(fx.goldenCoreGlow == null ? fxDefaults.goldenCoreGlow : Number(fx.goldenCoreGlow), 0.2, 2),
      goldenCoreColor: normalizeHexColor(fx.goldenCoreColor || fxDefaults.goldenCoreColor, fxDefaults.goldenCoreColor),
      goldenCoreHaloColor: normalizeHexColor(fx.goldenCoreHaloColor || fxDefaults.goldenCoreHaloColor, fxDefaults.goldenCoreHaloColor),
      goldenCoreOrbitCount: clampRange(Math.round(fx.goldenCoreOrbitCount == null ? fxDefaults.goldenCoreOrbitCount : Number(fx.goldenCoreOrbitCount)), 2, 10),
      goldenCoreOrbitSize: clampRange(fx.goldenCoreOrbitSize == null ? fxDefaults.goldenCoreOrbitSize : Number(fx.goldenCoreOrbitSize), 0.5, 2),
      goldenCoreDensity: clampRange(fx.goldenCoreDensity == null ? fxDefaults.goldenCoreDensity : Number(fx.goldenCoreDensity), 0.3, 2),
      goldenCoreSpin: clampRange(fx.goldenCoreSpin == null ? fxDefaults.goldenCoreSpin : Number(fx.goldenCoreSpin), 0, 2),
      goldenCoreStardust: clampRange(fx.goldenCoreStardust == null ? fxDefaults.goldenCoreStardust : Number(fx.goldenCoreStardust), 0, 1.5),
      goldenCoreBreath: clampRange(fx.goldenCoreBreath == null ? fxDefaults.goldenCoreBreath : Number(fx.goldenCoreBreath), 0, 1),
      treeCanopyTreeColor: normalizeHexColor(fx.treeCanopyTreeColor || fxDefaults.treeCanopyTreeColor, fxDefaults.treeCanopyTreeColor),
      treeCanopyStaffColor: normalizeHexColor(fx.treeCanopyStaffColor || fxDefaults.treeCanopyStaffColor, fxDefaults.treeCanopyStaffColor),
      treeCanopyNoteColor: normalizeHexColor(fx.treeCanopyNoteColor || fxDefaults.treeCanopyNoteColor, fxDefaults.treeCanopyNoteColor),
      treeCanopyGlow: clampRange(fx.treeCanopyGlow == null ? fxDefaults.treeCanopyGlow : Number(fx.treeCanopyGlow), 0.2, 2),
      treeCanopyWind: clampRange(fx.treeCanopyWind == null ? fxDefaults.treeCanopyWind : Number(fx.treeCanopyWind), 0, 2),
      treeCanopySway: clampRange(fx.treeCanopySway == null ? fxDefaults.treeCanopySway : Number(fx.treeCanopySway), 0, 2),
      treeCanopyStaffCount: clampRange(Math.round(fx.treeCanopyStaffCount == null ? fxDefaults.treeCanopyStaffCount : Number(fx.treeCanopyStaffCount)), 2, 7),
      treeCanopyNoteDensity: clampRange(fx.treeCanopyNoteDensity == null ? fxDefaults.treeCanopyNoteDensity : Number(fx.treeCanopyNoteDensity), 0.4, 2),
      treeCanopyNoteSize: clampRange(fx.treeCanopyNoteSize == null ? fxDefaults.treeCanopyNoteSize : Number(fx.treeCanopyNoteSize), 0.5, 2),
      treeCanopyBeatResponse: clampRange(fx.treeCanopyBeatResponse == null ? fxDefaults.treeCanopyBeatResponse : Number(fx.treeCanopyBeatResponse), 0, 2),
      treeCanopyAmbient: clampRange(fx.treeCanopyAmbient == null ? fxDefaults.treeCanopyAmbient : Number(fx.treeCanopyAmbient), 0, 2),
      heartPulseLineColor: normalizeHexColor(fx.heartPulseLineColor || fxDefaults.heartPulseLineColor, fxDefaults.heartPulseLineColor),
      heartPulseGlowColor: normalizeHexColor(fx.heartPulseGlowColor || fxDefaults.heartPulseGlowColor, fxDefaults.heartPulseGlowColor),
      heartPulseHeartColor: normalizeHexColor(fx.heartPulseHeartColor || fxDefaults.heartPulseHeartColor, fxDefaults.heartPulseHeartColor),
      heartPulseSpeed: clampRange(fx.heartPulseSpeed == null ? fxDefaults.heartPulseSpeed : Number(fx.heartPulseSpeed), 0.35, 2.4),
      heartPulseBeatResponse: clampRange(fx.heartPulseBeatResponse == null ? fxDefaults.heartPulseBeatResponse : Number(fx.heartPulseBeatResponse), 0, 2),
      heartPulseGlow: clampRange(fx.heartPulseGlow == null ? fxDefaults.heartPulseGlow : Number(fx.heartPulseGlow), 0.25, 2),
      heartPulseGrid: clampRange(fx.heartPulseGrid == null ? fxDefaults.heartPulseGrid : Number(fx.heartPulseGrid), 0, 1),
      heartPulseTrail: clampRange(fx.heartPulseTrail == null ? fxDefaults.heartPulseTrail : Number(fx.heartPulseTrail), 0.15, 1.5),
      heartPulseTitle: String(fx.heartPulseTitle == null ? fxDefaults.heartPulseTitle : fx.heartPulseTitle).slice(0, 28),
      heartPulseSubtitle: String(fx.heartPulseSubtitle == null ? fxDefaults.heartPulseSubtitle : fx.heartPulseSubtitle).slice(0, 36),
      heartPulseShowBpm: fx.heartPulseShowBpm !== false,
      heartPulseStatus: String(fx.heartPulseStatus == null ? fxDefaults.heartPulseStatus : fx.heartPulseStatus).slice(0, 24),
      heartPulseWaveform: /^(clinical|sine|double)$/.test(String(fx.heartPulseWaveform)) ? String(fx.heartPulseWaveform) : fxDefaults.heartPulseWaveform,
