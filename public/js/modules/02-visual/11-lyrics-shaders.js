...
function makeLyricShaderMaterial(mask, pal, motionProfile) {
  ...
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      uGlassSheen: { value: motionProfile.glassSheen || 0 },
    ...
    fragmentShader: [
      ...
      'uniform float uTime,uProgress,uTextMin,uTextMax,uOpacity,uFeather,uSolar,uSweep,uShimmer,uGlassSheen,uGlitch,uGlitchSlice,uGlitchChroma,uGlitchRate,uGlitchSeed,uGlitchBurst,uEdgeBoost,uActiveMix;',
      ...
      '  float sweepPhase = fract(uTime * (0.20 + uSweep * 0.34));',
      '  float sweepLine = (1.0 - smoothstep(0.0, 0.080, abs((uv.x + uv.y * 0.42) - (sweepPhase * 1.42 - 0.18)))) * activeMix;',
      '  float sweepCore = pow(sweepLine, 3.0);',
      '  float sheenPhase = fract(uTime * 0.070 + 0.35);',
      '  float sheenBand = (1.0 - smoothstep(0.0, 0.30, abs((uv.x - uv.y * 0.34) - (sheenPhase * 1.55 - 0.22)))) * uGlassSheen * activeMix;',
      ...
      '  color += mix(uHiColor, vec3(1.0), 0.35) * sweepCore * min(1.35, uSweep * 0.55) * 0.40 * activeMix;',
      '  color += mix(uGlowColor, vec3(1.0), 0.55) * sheenBand * (0.14 + filled * 0.26);',
      '  color += vec3(0.032) * uGlassSheen * activeMix;',
