...
function lyricQualityDisposeTexture(texture) {
  ...
  else if (!texture.userData || !texture.userData.__MOMusicDisposed) {
...
function lyricQualityOwnerActive(data, row) {
  return !!(
    data && data.__MOMusicLyricQualityDisposed !== true &&
...
function updateLyricQualityStats(tier) {
  ...
  window.__MOMusicLyricQualityStats = {
...
function disposeLyricQualityOwner(data) {
  if (!data) return;
  data.__MOMusicLyricQualityDisposed = true;
...
function resetLyricRenderUploadFrameBudget(deferQualityFinalize) {
  ...
  if (typeof window !== 'undefined') {
    window.__MOMusicLyricUploadBudgetStats = {
