...
var DESKTOP_ICON_SHIELD_TARGETS = [
  ...
  { selector: '#audio-fx-modal', kind: 'modal', visual: true },
...
function updateDesktopModeControl(status) {
  ...
  if (softwareLockButton) {
    ...
    softwareLockButton.title = softwareLocked ? '恢复 MOMusic 操作' : '暂时把操作交给 Windows 桌面';
  }
...
function currentDesktopSongMeta() {
  ...
  return {
    title: song.name || song.title || 'MOMusic',
...
function currentDesktopLyricSnapshot() {
  ...
  return { text: normalizeDesktopLyricText(currentDesktopSongMeta().title || 'MOMusic'), progress: 0, progressSpan: 4.8 };
}
...
function updateDesktopWallpaperRuntimeControls(status) {
  ...
  if (toggle) {
    ...
    toggle.title = !supported
      ? '当前系统不支持完整桌面模式'
      : (attaching ? '正在切换完整桌面模式' : '把完整 MOMusic 放到 Windows 桌面；右上角控制器可显示或隐藏桌面图标，Esc 退出');
  }
...
function revealDesktopWallpaperUiOnActivation(enabled, interactive) {
  ...
  // Full desktop mode is the complete MOMusic workspace. Do not inherit the
