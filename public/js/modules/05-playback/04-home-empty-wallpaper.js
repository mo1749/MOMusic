...
async function playHomeDashboardDaily() {
  homeForcedOpen = false;
  homeSuppressed = false;
  setHomeControlsLocked(false);
  ...
  var platforms = [];
  var check = function (prov) { if (hasPlatformLogin(prov)) platforms.push(prov); };
  check('netease'); check('qq'); check('kugou'); check('qishui'); check('spotify');
  if (!platforms.length) { showLoginModal({ source: 'home-daily' }); return; }
  if (platforms.length > 1) {
    openHomePlatformRecommendations(platforms[0]);
    return;
  }
  var source = platforms[0];
  await loadHomeDiscover(true);
  openHomePlatformRecommendations(source);
}
