...
var HOME_DASHBOARD_VIDEO_DB_NAME = 'MOMusic-home-dashboard-video-v1';
var HOME_DASHBOARD_VIDEO_STORE = 'media';
var HOME_DASHBOARD_VIDEO_BLOB_ID = 'home-hero-video';
var HOME_DASHBOARD_VIDEO_META_KEY = 'MOMusic-home-dashboard-video-meta-v1';
...
var HOME_DASHBOARD_REVIEW_DEFAULTS = [
  ...
  { text: '让今天的声音，从你喜欢的地方开始。', source: 'MOMusic' },
];
...
function homeDashboardGeneratedCover(title, label, tone) {
  ...
  var sub = homeDashboardSvgText(String(label || 'MOMusic').toUpperCase().slice(0, 14));
...
function homeDashboardReadReviews() {
  try {
    var saved = JSON.parse(localStorage.getItem('MOMusic-daily-review-quotes-v1') || '[]');
...
function homeDashboardSelectedReview() {
  ...
  if (!reviews.length) return { text: '让今天的声音，从你喜欢的地方开始。', source: 'MOMusic' };
...
function renderHomeDashboardDiscovery() {
  ...
  root.innerHTML = homeDashboardDiscoveryCache.map(function (song, index) {
    ...
    return '<button class="home-discovery-song" type="button" onclick="playHomeDashboardDiscoverySong(' + index + ')">' +
      ...
      '<span class="home-discovery-song-artist">' + escHtml(homeDashboardSubtitle(song) || 'MOMusic 推荐') + '</span></span></button>';
  }).join('');
}
...
function homePlatformRecommendationFeedConfig(source) {
  return {
    ...
    qq: {
      ...
      sectionTitle: 'QQ 我的喜欢',
      cardLabel: 'QQ 喜欢的歌曲',
      readyText: '来自 QQ 音乐的喜欢的歌曲',
      playlistName: 'QQ 喜欢的歌曲',
    },
    spotify: {
...
function openHomePlatformRecommendations(preferredSource) {
  ...
  var defaultSource = loginStatus && loginStatus.loggedIn
    ? 'netease'
    : (qqLoginStatus && (qqLoginStatus.loggedIn || qqLoginStatus.preview)
      ? 'qq'
