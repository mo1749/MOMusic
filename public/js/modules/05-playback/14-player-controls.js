...
function playbackStallRecoveryOwnerStillCurrent(media, src, token, recoverySerial, queueKey) {
  ...
  if (queueKey && String(media.__MOMusicQueueItemKey || '') !== queueKey) return false;
...
function schedulePlaybackStallRecovery(reason, opts) {
  ...
  var queueKey = String(opts.ownerQueueItemKey || media.__MOMusicQueueItemKey || '');
  if (queueKey && String(media.__MOMusicQueueItemKey || '') !== queueKey) return;
...
function playbackMediaMatchesCurrentQueueItem(media) {
  ...
  var mediaKey = String(media.__MOMusicQueueItemKey || '');
