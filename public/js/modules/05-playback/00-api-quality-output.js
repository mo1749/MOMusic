  ...
  var workflowHtml =
    '<div class="audio-route-graph' + (bridgeEnabled ? ' bridge-on' : '') + '">' +
      ...
      '<div class="audio-flow-source workflow-node" data-audio-node="player">' +
        '<span class="route-node-kicker">SOURCE</span><span class="route-node-icon">MR</span><span class="route-node-text"><b>MOMusic Player</b><small>' + escHtml(summaryText) + '</small></span><span class="audio-source-meter" aria-hidden="true"><i></i><i></i><i></i><i></i></span><span class="flow-port out" data-audio-route-source="player" title="MOMusic 输出"></span>' +
      '</div>' +
...
function bindAudioOutputMirrorEvents(media) {
  if (!media || media._MOMusicAudioMirrorBound) return;
  media._MOMusicAudioMirrorBound = true;
...
function syncAudioOutputMirrors(reason) {
  ...
  ids.forEach(function (id) {
    ...
    if (mirror._MOMusicSinkId !== id || !mirror._MOMusicSinkReady) {
      mirror._MOMusicSinkId = id;
      if (!mirror._MOMusicSinkBusy) {
        mirror._MOMusicSinkBusy = true;
        Promise.resolve(applyAudioOutputMirrorSink(mirror, id)).then(function (ok) {
          mirror._MOMusicSinkBusy = false;
          mirror._MOMusicSinkReady = !!ok;
    ...
    if (!mirror._MOMusicSinkReady) return;
