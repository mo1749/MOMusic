...
function replaceAudioElementForGraphRecovery(reason, opts) {
  ...
  var queueItemKey = oldAudio.__MOMusicQueueItemKey;
  ...
  audio.__MOMusicQueueItemKey = queueItemKey;
...
function initAudio() {
  ...
  var sourceUsesCapture = !!(keepSource && source.__MOMusicUsesCapture);
  disconnectAudioGraphNodes(keepSource);
  if (!source) {
    var forceCapture = !!audio.__MOMusicForceCaptureSource;
    ...
    if (!forceCapture && !audio.__MOMusicMediaSourceBound && audioCtx.createMediaElementSource) {
      try {
    ...
    if (!forceCapture && !mediaSource && audio.__MOMusicMediaSourceBound) {
    ...
    source.__MOMusicUsesCapture = !mediaSource;
    audioSourceMedia = audio;
    ...
    if (mediaSource) audio.__MOMusicMediaSourceBound = true;
...
function rebuildPlaybackGraphWithCapture(reason) {
  ...
  if (audio.__MOMusicMediaSourceBound) {
    if (!source || source.__MOMusicUsesCapture || audioSourceMedia !== audio) return false;
...
function schedulePlaybackAnalyserRecovery(reason) {
  ...
  [720, 1600, 2800].forEach(function (delay) {
    setTimeout(function () {
      ...
      if (source && !source.__MOMusicUsesCapture && audio.captureStream) {
