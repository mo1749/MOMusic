var CUEFIELD_AUTOMIX_STORE_KEY = 'MOMusic-cuefield-automix-v1';
...
function claimCuefieldPreparedAudioForPlayback(media) {
  ...
  cancelCuefieldMediaFade();
  if (media.__MOMusicPreparedAudioGraph) media.__MOMusicPreparedAudioGraph.adopted = true;
  if (media === cuefieldAutoMixPreparedAudio) cuefieldAutoMixPreparedAudio = null;
  return true;
}

function disposeCuefieldPreparedAudioGraph(media) {
  var graph = media && media.__MOMusicPreparedAudioGraph;
  ...
  try { delete media.__MOMusicPreparedAudioGraph; } catch (_) { }
}

function stopCuefieldPreparedAudio(media) {
  media = media || cuefieldAutoMixPreparedAudio;
  if (!media) return;
  // Once the preloaded B deck has become MOMusic's active deck it no longer
...
function cuefieldCreatePreparedAudioGraph(media) {
  if (!media || media.__MOMusicPreparedAudioGraph) return media && media.__MOMusicPreparedAudioGraph || null;
  var graph = null;
  try {
    ...
    media.__MOMusicMediaSourceBound = true;
    ...
    media.__MOMusicPreparedAudioGraph = graph;
    return graph;
  } catch (error) {
    if (graph) {
      [graph.source, graph.analyser, graph.beatAnalyser, graph.gainNode].forEach(function (node) {
    ...
    if (media && media.__MOMusicMediaSourceBound) media.__MOMusicPreparedGraphFailed = true;
    try { delete media.__MOMusicPreparedAudioGraph; } catch (_) { }
...
function prepareCuefieldPendingAudio(pending) {
  ...
  if (media.__MOMusicPreparedGraphFailed) {
...
async function runCuefieldTimeline(pending, nextMedia, context) {
  ...
  pending.executionFallback = nextMedia.__MOMusicPreparedAudioGraph ? 'shared-context-gain' : 'direct-volume-fallback';
...
function noteCuefieldAutoMixOutgoingEnded(media, token, index) {
  if (!media) return false;
  media.__MOMusicCuefieldEndedDeferredToken = Number(token);
  media.__MOMusicCuefieldEndedDeferredIndex = Number(index);
  return true;
}
...
function recoverCuefieldAutoMixEndedOutgoing(pending, context, reason) {
  ...
  if (outgoing.__MOMusicCuefieldEndedRecoveryToken === token) return true;
  outgoing.__MOMusicCuefieldEndedRecoveryToken = token;
