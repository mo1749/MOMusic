...
async function mainTarget(timeoutMs = 15000) {
  ...
  throw new Error('MOMusic CDP target was not ready');
}
...
using System.Text;
public static class MOMusicExplorerUntouchedProbe {
...
$region = [IntPtr]::Zero
try {
  try { $previousDpi = [MOMusicExplorerUntouchedProbe]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $desktopHost = [IntPtr]::Zero
  $view = [IntPtr]::Zero
  $list = [IntPtr]::Zero
  $expected = [IntPtr]::new([Int64]${expected})
  if ($expected -ne [IntPtr]::Zero) {
    if (-not [MOMusicExplorerUntouchedProbe]::IsWindow($expected)) { throw 'EXPECTED_EXPLORER_LIST_NOT_FOUND' }
    if ([MOMusicExplorerUntouchedProbe]::ClassName($expected) -ne 'SysListView32') { throw 'EXPECTED_EXPLORER_LIST_CLASS_CHANGED' }
    $list = $expected
    $view = [MOMusicExplorerUntouchedProbe]::GetParent($list)
    $desktopHost = [MOMusicExplorerUntouchedProbe]::GetAncestor($list, 2)
  } elseif (-not [MOMusicExplorerUntouchedProbe]::FindDesktop([ref]$desktopHost, [ref]$view, [ref]$list)) {
    throw 'EXPLORER_DESKTOP_LIST_NOT_FOUND'
  }
  if ([MOMusicExplorerUntouchedProbe]::ClassName($list) -ne 'SysListView32') { throw 'EXPLORER_DESKTOP_LIST_INVALID' }
  $rect = New-Object MOMusicExplorerUntouchedProbe+RECT
  if (-not [MOMusicExplorerUntouchedProbe]::GetWindowRect($list, [ref]$rect)) { throw 'EXPLORER_DESKTOP_RECT_FAILED' }
  $region = [MOMusicExplorerUntouchedProbe]::CreateRectRgn(0, 0, 0, 0)
  if ($region -eq [IntPtr]::Zero) { throw 'EXPLORER_DESKTOP_REGION_ALLOC_FAILED' }
  $regionType = [MOMusicExplorerUntouchedProbe]::GetWindowRgn($list, $region)
  $regionBox = New-Object MOMusicExplorerUntouchedProbe+RECT
  $regionBoxType = 0
  if ($regionType -gt 0) { $regionBoxType = [MOMusicExplorerUntouchedProbe]::GetRgnBox($region, [ref]$regionBox) }
  [uint32]$layerColorKey = 0
  [byte]$layerAlpha = 0
  [uint32]$layerFlags = 0
  $layerAvailable = [MOMusicExplorerUntouchedProbe]::GetLayeredWindowAttributes($list, [ref]$layerColorKey, [ref]$layerAlpha, [ref]$layerFlags)
  $layerError = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
  [uint32]$processId = 0
  $threadId = [MOMusicExplorerUntouchedProbe]::GetWindowThreadProcessId($list, [ref]$processId)
  $style = [MOMusicExplorerUntouchedProbe]::GetWindowLongU32($list, -16)
  $exStyle = [MOMusicExplorerUntouchedProbe]::GetWindowLongU32($list, -20)
  $backgroundResult = [IntPtr]::Zero
  if ([MOMusicExplorerUntouchedProbe]::SendMessageTimeout($list, 0x1000, [IntPtr]::Zero, [IntPtr]::Zero, 0x0002, 500, [ref]$backgroundResult) -eq [IntPtr]::Zero) {
    throw 'EXPLORER_DESKTOP_BACKGROUND_TIMEOUT'
  }
  $background = [MOMusicExplorerUntouchedProbe]::IntPtrU32($backgroundResult)
  [pscustomobject]@{
    handle = $list.ToInt64().ToString()
    className = [MOMusicExplorerUntouchedProbe]::ClassName($list)
    parentWindowId = $view.ToInt64().ToString()
    parentClassName = [MOMusicExplorerUntouchedProbe]::ClassName($view)
    rootWindowId = $desktopHost.ToInt64().ToString()
    rootClassName = [MOMusicExplorerUntouchedProbe]::ClassName($desktopHost)
    ...
    enabled = [MOMusicExplorerUntouchedProbe]::IsWindowEnabled($list)
    visible = [MOMusicExplorerUntouchedProbe]::IsWindowVisible($list)
  ...
  if ($region -ne [IntPtr]::Zero) { [MOMusicExplorerUntouchedProbe]::DeleteObject($region) | Out-Null }
  if ($previousDpi -ne [IntPtr]::Zero) { try { [MOMusicExplorerUntouchedProbe]::SetThreadDpiAwarenessContext($previousDpi) | Out-Null } catch { } }
}`;
...
using System.Runtime.InteropServices;
using System.Text;
public static class MOMusicDesktopRouteHitTest {
...
$previousDpi = [IntPtr]::Zero
try {
  try { $previousDpi = [MOMusicDesktopRouteHitTest]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  ...
  for ($depth = 0; $current -ne [IntPtr]::Zero -and $depth -lt 12; $depth++) {
    ...
    [MOMusicDesktopRouteHitTest]::GetClassName($current, $className, $className.Capacity) | Out-Null
    [MOMusicDesktopRouteHitTest]::GetWindowText($current, $title, $title.Capacity) | Out-Null
    $chain += [pscustomobject]@{ handle = $current.ToInt64().ToString(); className = $className.ToString(); title = $title.ToString() }
    $point = New-Object MOMusicDesktopRouteHitTest+POINT
    $point.X = ${Math.round(x)}
    $point.Y = ${Math.round(y)}
    if (-not [MOMusicDesktopRouteHitTest]::ScreenToClient($current, [ref]$point)) { break }
    ...
    $child = [MOMusicDesktopRouteHitTest]::ChildWindowFromPointEx($current, $point, 0x0007)
    if ($child -eq [IntPtr]::Zero -or $child -eq $current) { break }
    $current = $child
  }
  $chain | ConvertTo-Json -Compress
} finally {
  if ($previousDpi -ne [IntPtr]::Zero) { try { [MOMusicDesktopRouteHitTest]::SetThreadDpiAwarenessContext($previousDpi) | Out-Null } catch { } }
}`;
...
async function main() {
  ...
  try {
    ...
    assert.equal(enabled.status && enabled.status.ignoreMouseEvents, false,
      'unlocked complete MOMusic surface is still globally click-through');
    ...
    assert(enabled.naturalHud.bottomVisible || enabled.naturalHud.homeVisible,
      `full desktop started without a natural MOMusic HUD: ${JSON.stringify(enabled.naturalHud)}`);
    ...
    assert(iconHit.some((item) => item.className === 'SysListView32'),
      `real Explorer icon did not remain above MOMusic: ${JSON.stringify(iconHit)}`);
    ...
    assert.equal(naturalInputStatus.ignoreMouseEvents, false,
      'unlocked blank area incorrectly disabled the complete MOMusic input surface');
    ...
    for (const route of [
      ...
      assert.equal(status.ignoreMouseEvents, false,
        `desktop pointer route unexpectedly disabled MOMusic input: ${JSON.stringify(route)}`);
    ...
    assert.equal(unlockedBlank.ignoreMouseEvents, false,
      'unlock did not restore natural MOMusic input outside the controller');
    ...
    assert(persistentHud.bottomVisible || persistentHud.homeVisible,
      `full desktop lost its complete MOMusic HUD after the ordinary auto-hide path: ${JSON.stringify(persistentHud)}`);
    ...
    console.log(JSON.stringify({
      ...
      manualHardwareChecksStillRequired: [
        ...
        'Lock MOMusic, move back into the top-right reveal corridor, and use the same switch to unlock it.',
