...
async function waitForMainTarget(timeoutMs = 20000) {
  ...
  throw new Error(`MOMusic CDP page was not ready: ${lastError && lastError.message || 'no target'}`);
}
...
async function main() {
  ...
  try {
    ...
    title = String(reset.title || 'MOMusic');
    ...
    assert(originalCandidates.length > 0, 'the top-level MOMusic HWND was not found before desktop mode');
    ...
    assert.strictEqual(readWindowTree('MOMusic Desktop Wallpaper').length, 0, 'legacy overlay wallpaper window was created');
    assert.strictEqual(readWindowTree('MOMusic WE Glass Refraction').length, 0, 'forbidden second WE glass window was created');
