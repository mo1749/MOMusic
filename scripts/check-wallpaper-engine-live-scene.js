...
async function main() {
  ...
  assert(target && target.webSocketDebuggerUrl, 'MOMusic CDP page target was not found');
  ...
  assert(result.applied.runtimeStatus && result.applied.runtimeStatus.sourceWindowEmbedded === true, 'Wallpaper Engine source window was not docked directly behind MOMusic');
  assert.strictEqual(result.applied.runtimeStatus.sourceWindowAligned, true, 'Wallpaper Engine source window is not pixel-aligned with MOMusic');
  ...
  assert(Math.abs(result.applied.videoWidth - result.applied.runtimeStatus.width) <= 2, 'native Scene capture width does not match the MOMusic compositor');
  assert(Math.abs(result.applied.videoHeight - result.applied.runtimeStatus.height) <= 2, 'native Scene capture height does not match the MOMusic compositor');
