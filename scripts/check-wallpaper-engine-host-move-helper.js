...
async function main() {
  assert(Number.isInteger(hostPid) && hostPid > 0, 'A MOMusic host PID is required');
  const targets = await fetch(`http://127.0.0.1:${port}/json/list`).then((response) => response.json());
  const target = targets.find((item) => item.type === 'page' && /127\.0\.0\.1/.test(item.url || ''));
  assert(target && target.webSocketDebuggerUrl, 'MOMusic renderer target was not found');
...
$env:TEMP='D:\\MOMusicCache\\native-helper-temp'
$env:TMP=$env:TEMP
Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
public static class MOMusicQaHostMove {
...
$rect=New-Object MOMusicQaHostMove+RECT
if (-not [MOMusicQaHostMove]::GetWindowRect($app.MainWindowHandle,[ref]$rect)) { throw 'GetWindowRect failed' }
if (-not [MOMusicQaHostMove]::SetWindowPos($app.MainWindowHandle,[IntPtr]::Zero,$rect.Left+80,$rect.Top+40,0,0,0x0015)) { throw 'SetWindowPos failed' }
`;
