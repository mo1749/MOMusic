...
function checkWallpaperEngineImportGuard() {
  ...
  if (!/DwmRegisterThumbnail/.test(dwmSurfaceBlock)
    ...
    || /MOMusic WE Glass Refraction/.test(dwmSurfaceBlock)
    ...
    || !/source\.name \|\| ''\) === 'MOMusic WE DWM Surface'/.test(runtimeText)
    || !/MOMusic-wallpaper-engine-glass-surface/.test(mainText)
    || !/kind: 'dwm-glass'/.test(mainText)
    || !/prepareWallpaperEngineRendererGlassCapture/.test(mainText)
    || !/__MOMusicPrepareWallpaperEngineGlassCapture/.test(rendererText)
  ...
  const wallpaperCaptureResultBlock = mainText.slice(
    mainText.indexOf("ipcMain.handle('MOMusic-wallpaper-engine-capture-result'"),
    mainText.indexOf("ipcMain.handle('MOMusic-wallpaper-engine-stop-scene'")
  );
  ...
  if (!/MOMusic_NATIVE_TEMP_DIR/.test(systemMemoryText) || !/setNativeTempPath/.test(systemMemoryText) || !/TEMP:\s*ensureNativeTempPath\(\)/.test(systemMemoryText) || !/NATIVE_HELPER_TEMP_PATH/.test(mainText) || !/nativeTempPath:\s*NATIVE_HELPER_TEMP_PATH/.test(mainText) || !/TEMP:\s*this\.nativeTempPath/.test(runtimeText)) {
    fail('PowerShell Add-Type helpers must use the app-owned stable native temp directory');
  }
  if (/MOMusic_WE_QA_CLEANUP/.test(mainText) || /taskkill\.exe[\s\S]{0,120}wallpaper(?:32|64)/i.test(mainText) || /x:\s*-16000[\s\S]{0,80}y:\s*-16000/.test(mainText) || !/width:\s*Math\.max\(640,[\s\S]{0,220}physicalBounds\.width[\s\S]{0,220}x:\s*physicalBounds\.x,[\s\S]{0,80}y:\s*physicalBounds\.y/.test(mainText)) {
  ...
  if (!/appQuitCleanupPromise/.test(mainText) || !/event\.preventDefault\(\)/.test(mainText) || !/Promise\.race\(\[runtimeCleanup, timeoutCleanup\]\)/.test(mainText)) {
    fail('Wallpaper Engine shutdown must wait briefly for the MOMusic-owned source window to close');
  }
...
function checkDesktopWallpaperModeGuard() {
  ...
  if ((mainText.match(/new WallpaperEngineRuntime/g) || []).length !== 1
    ...
    || /MOMusic WE Glass Refraction/.test(wallpaperEngineRuntimeText)
  ...
  if (!createWallpaperWindowBlock
    ...
    fail('entering full desktop mode must expose the complete MOMusic HUD without creating or requiring a legacy fallback wallpaper');
  }
  if (!/MOMusic-wallpaper-set-enabled/.test(mainText)
    || !/MOMusic-wallpaper-get-status/.test(mainText)
  ...
  if (!/new FullDesktopModeRuntime/.test(mainText)
    || !/MOMusic-full-desktop-icon-shields/.test(mainText + preloadText)
    ...
    fail('full desktop coexistence must preserve the visible MOMusic HUD, survive native watcher/DWM retries, and remain below the exactly restored Explorer icon plane');
  }
  if (!/id="desktop-mode-control-dock"/.test(htmlText)
    ...
    || !/MOMusic-full-desktop-set-software-lock/.test(mainText)
    || !/MOMusic-full-desktop-set-icons-visible/.test(mainText)
    || !/MOMusic-full-desktop-request-keyboard-focus/.test(mainText)
    || !/MOMusic-full-desktop-pointer-route/.test(mainText)
  ...
  if (!/fullDesktopModeRuntime\.dispose\('app-before-quit'\)/.test(shutdownBlock)
    ...
    fail('shutdown must detach the complete MOMusic HUD before disposing the single WE DWM chain, with no legacy backdrop cleanup path');
  }
  ...
  console.log('[OK] Legacy canvas backdrop is forbidden; the complete MOMusic HUD, recoverable software lock, desktop-icon switch, click-outside close, Escape exit, native watcher recovery, single WE DWM layering, and ordered cleanup are guarded.');
}
...
function checkPersistentCacheStorageGuard() {
  ...
  if (!/const CACHE_SETTINGS_FILE/.test(mainText) || !/const LYRIC_CACHE_MAX_BYTES = 96 \* 1024 \* 1024/.test(mainText) || !/function defaultCacheRootPath\(\)/.test(mainText) || !/path\.join\(dDrive, 'MOMusicCache'\)/.test(mainText) || setNameAt < 0 || firstUserDataLookupAt < 0 || setNameAt > firstUserDataLookupAt || !/const STABLE_USER_DATA_PATH = path\.join\(app\.getPath\('appData'\), APP_NAME\)/.test(mainText) || !/app\.setPath\('userData', STABLE_USER_DATA_PATH\)/.test(mainText) || !/app\.setPath\('sessionData', chromiumSessionDataPath\(cacheSettings\)\)/.test(mainText) || !/const currentChromiumPath = app\.getPath\('sessionData'\)/.test(mainText) || !/MOMusic_BEAT_CACHE_DIR = cacheSettings\.beatmapsPath/.test(mainText) || !/nativePath:\s*path\.join\(rootPath, 'native-helper-temp'\)/.test(mainText) || !/const NATIVE_HELPER_TEMP_PATH = INITIAL_CACHE_SETTINGS\.nativePath/.test(mainText) || !/activeWallpaperEnginePath/.test(mainText) || !/wallpaperEngineBytes/.test(mainText)) {
  ...
  if (!/MOMusic-cache-get-settings/.test(mainText) || !/MOMusic-cache-set-settings/.test(mainText) || !/MOMusic-cache-read-lyric/.test(mainText) || !/MOMusic-cache-write-lyric/.test(mainText) || !/crypto\.createHash\('sha256'\)/.test(mainText) || !/pruneLyricCache/.test(mainText)) {
...
function checkQishuiProviderGuard() {
  ...
  if (!/function readSavedQishuiCookieHeader\(\)/.test(desktopMainText) || !/const saved = readSavedQishuiCookieHeader\(\)/.test(desktopMainText) || !/savedStatus = await handleQishuiStatus\(saved\.cookie\)/.test(desktopMainText) || !/savedStatus\.loggedIn && savedStatus\.webSession/.test(desktopMainText) || !/persistedSession: true/.test(desktopMainText)) {
    fail('Qishui desktop login must validate the cached MOMusic copy when the official client is temporarily unavailable');
  }
...
function checkPlaybackControlBadgesGuard() {
  ...
  const accountPillGlassSurfaceOk =
    ...
    /html\.control-glass-svg-ok\s+\.top-account-pill::before\s*\{[\s\S]*?url\(#MOMusic-account-pill-glass-filter\)/.test(cssText);
  const accountPillDirectSvgFilter =
    /html\.control-glass-svg-ok\s+\.top-account-pill\s*\{[\s\S]*?url\(#MOMusic-account-pill-glass-filter\)/.test(cssText) ||
  ...
  const accountFilterText = (indexText.match(/<filter id="MOMusic-account-pill-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
...
async function checkProviderFallbackTerminalStateGuard() {
  ...
  if (!/async function skipFailedQueueItem/.test(fallbackText) || !/skipShuffleOrder:\s*true/.test(fallbackText) || !/return nextStarted === true/.test(fallbackText) || !/function settleSourceFallbackTerminal/.test(fallbackText) || !/audio\.removeAttribute\('src'\)/.test(fallbackText) || !/audio\.__MOMusicQueueItemKey = ''/.test(fallbackText)) {
...
function checkSearchGlassEntranceGuard() {
  ...
  const searchPillDirectSvg = /html\.control-glass-svg-ok\s+\.search-mode-tabs button,[ \t]*\r?\nhtml\.control-glass-svg-ok\s+\.search-history-chip\s*\{[\s\S]{0,260}backdrop-filter:\s*url\(#MOMusic-search-pill-glass-filter\)\s+saturate\(1\)\s*!important[\s\S]{0,220}-webkit-backdrop-filter:\s*url\(#MOMusic-search-pill-glass-filter\)\s+saturate\(1\)\s*!important/.test(cssText);
  ...
  const searchBoxUsesSavedRgbGlassSurface =
    ...
    /html\.control-glass-svg-ok\s+#search-area\.peek\s+#search-box\s*\{[\s\S]{0,180}backdrop-filter:\s*url\(#MOMusic-search-box-glass-filter\)\s+saturate\(1\)\s*!important[\s\S]{0,180}-webkit-backdrop-filter:\s*url\(#MOMusic-search-box-glass-filter\)\s+saturate\(1\)\s*!important/.test(cssText) &&
    ...
    !/html\.control-glass-svg-ok\s+#search-box::before\s*\{[\s\S]{0,260}url\(#MOMusic-search-box-glass-filter\)/.test(cssText) &&
  ...
  const searchPillUsesSavedRgbGlassSurface =
    /html\.control-glass-svg-ok\s+\.search-mode-tabs button,[ \t]*\r?\nhtml\.control-glass-svg-ok\s+\.search-history-chip\s*\{[\s\S]{0,180}background:\s*var\(--saved-button-glass-bg\)\s*!important[\s\S]{0,180}border-color:\s*transparent\s*!important[\s\S]{0,180}box-shadow:\s*var\(--saved-button-glass-shadow\)\s*!important[\s\S]{0,220}url\(#MOMusic-search-pill-glass-filter\)/.test(cssText);
  ...
  const searchBoxFilterText = (indexText.match(/<filter id="MOMusic-search-box-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
  const searchPillFilterText = (indexText.match(/<filter id="MOMusic-search-pill-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
  ...
  const searchGlassUsesSavedRgbMapOk =
    ...
    !/MOMusic-search-box-glass-filter|MOMusic-search-pill-glass-filter/.test(chromaticOffsetFunctionText) &&
...
async function checkProviderAuthCookiePathGuard() {
  ...
  if (!/function reportWindowCreationFailure\(context, error\)/.test(mainText) || !/dialog\.showErrorBox\('MOMusic 启动失败'/.test(mainText)) {
...
function checkPlaybackResumeRecoveryGuard() {
  ...
  if (
    ...
    || !/__MOMusicTrackSwitchToken\) !== Number\(trackSwitchToken\)/.test(progressText)
...
function checkInternalBetaPackagingGuard() {
  ...
  if ((beta.appId || '') !== 'com.MOMusic.beat.internal' || (MOMusic.appUserModelId || '') !== 'com.MOMusic.beat.internal') {
    fail('internal beta must use an isolated app id/AppUserModelID');
  }
  if (MOMusic.runtimeName !== 'MOMusic_Beat' || update.disabled !== true || update.provider !== 'none') {
  ...
  if (!beta.nsis || beta.nsis.include !== 'build/installer-internal-beta.nsh' || !/MOMusic_Beat-v\$\{version\}-灰度内测版/.test(beta.nsis.artifactName || '')) {
  ...
  if (!/MOMusic_INSTALL_DIR_NAME "MOMusic_Beat"/.test(wrapperText) || !/禁止传播/.test(wrapperText) || !/installer\.nsh/.test(wrapperText)) {
    fail('internal beta NSIS wrapper must define MOMusic_Beat and the no-redistribution notice');
  }
  const installerText = fs.readFileSync(path.join(appRoot, 'build', 'installer.nsh'), 'utf8');
  if (!/MOMusic_INSTALL_DIR_NAME/.test(installerText) || !/MOMusic_INSTALL_NOTICE/.test(installerText)) {
...
function checkSonicTopographyPresetGuard() {
  ...
  if (!/sonic-topography-preset\.js/.test(loaderText) || !/var INDEX = 7/.test(sonicText) || !/function deriveTerrainGridSettings/.test(sonicText) || !/TERRAIN_BASE_SIZE = 168/.test(sonicText) || !/TERRAIN_MAX_GRID_SIZE = 224/.test(sonicText)) {
    fail('Sonic Topography preset must load as a bounded MOMusic-native port of the latest GitHub visual layer');
  }
  ...
  if (/sonic-workshop-preset\.js/.test(loaderText)
    || /MOMusicSonicWorkshop/.test(mainLoopText + presetGridText)
  ...
  if (/getUserMedia|getDisplayMedia|desktopCapturer|MediaStream/.test(sonicAudioText + mainLoopText)) {
    fail('Sonic realtime audio must reuse MOMusic analyser data instead of requesting system or microphone capture');
  }
  ...
  if (!/MOMusicSonicTopography\.update/.test(mainLoopText) || !/visual\.sonic-topography/.test(mainLoopText) || !/MOMusicSonicTopography\.onPresetChange/.test(presetGridText) || !/MOMusicSonicTopography\.pointerRipple/.test(pointerText)) {
...
function checkPlaylistPanelTriggerGuard() {
  ...
  if (!/--playlist-panel-open-ms:\s*var\(--MOMusic-playlist-panel-open-ms,\s*280ms\)/.test(cssText) || !/--playlist-panel-close-ms:\s*var\(--MOMusic-playlist-panel-close-ms,\s*180ms\)/.test(cssText) || !/setPlaylistPanelCssVar\('--MOMusic-playlist-panel-open-ms'/.test(fxRuntimeText) || !/setPlaylistPanelCssVar\('--MOMusic-playlist-panel-close-ms'/.test(fxRuntimeText)) {
...
const appRoot = process.env.MOMusic_QA_APP_ROOT;
const qaPreload = process.env.MOMusic_QA_PRELOAD;
const pagePath = path.join(appRoot, 'public', 'index.html');
const logs = [];

function finish(code, payload) {
  console.log('MOMusic_QA_RESULT:' + JSON.stringify(payload));
...
app.whenReady().then(async () => {
  ...
  const result = await win.webContents.executeJavaScript(\`
    (async () => {
      ...
      const runtime = typeof window.__MOMusicPerfSnapshot === 'function' ? window.__MOMusicPerfSnapshot() : null;
      const perf = window.__MOMusicPerf && typeof window.__MOMusicPerf.snapshot === 'function'
        ? window.__MOMusicPerf.snapshot()
        : null;
        ...
        try {
          ...
          [2, 3, 4].forEach(tier => {
            ...
            rows.push({
              ...
              markedQuality: !!(built && built.texture && built.texture.userData && built.texture.userData.__MOMusicLyricQuality)
            });
          });
      ...
      async function inspectLyricQualityCacheStability() {
        ...
        try {
          ...
          const immediateSame = lyricQualityCurrentMap(anchor) === oldTexture && !oldTexture.userData.__MOMusicDisposed;
          let sawBase = false;
          ...
          const handoffOk = startedOverNewBudget && immediateSame && !sawBase && anchor.qualityTier === 3 && allTierThree && oldTexture.userData.__MOMusicDisposed && lyricQualityCurrentMap(anchor) === anchor.qualityTexture;
          const newTier = anchor.qualityTier;
          const uploadOk = !window.__MOMusicLyricUploadBudgetStats || Number(window.__MOMusicLyricUploadBudgetStats.maxConsumed) <= 1;
          resetLyricRenderUploadFrameBudget(true);
          updateLyricRowLayers(data, updateOptions);
          ...
          const noDisposeResurrection = data.__MOMusicLyricQualityDisposed === true &&
        ...
        try {
          ...
          for (const target of targets) {
            ...
            while (performance.now() < deadline) {
              ...
              const uploadStats = window.__MOMusicLyricUploadBudgetStats || {};
        ...
        try {
          ...
          const sampleContinuousMotion = () => {
            ...
            if (previousContinuousSample) {
              ...
              const renderFrame = Number(window.__MOMusicLyricUploadBudgetStats && window.__MOMusicLyricUploadBudgetStats.frame) || 0;
            ...
            previousContinuousSample = {
              ...
              visualOffset,
              targetVirtualIndex,
              renderFrame: Number(window.__MOMusicLyricUploadBudgetStats && window.__MOMusicLyricUploadBudgetStats.frame) || 0
            };
      ...
      async function inspectSearchGlassEntrance() {
        ...
        const readSearchBoxGlassStyle = () => {
          ...
          const boxFilter = document.getElementById('MOMusic-search-box-glass-filter');
          const pillFilter = document.getElementById('MOMusic-search-pill-glass-filter');
          const readHref = img => {
            if (!img) return '';
        ...
        const searchBoxFilterLooksLikeSavedRgbGlass = value => String(value || '').includes('MOMusic-search-box-glass-filter') && String(value || '').includes('saturate(1)');
        const searchPillFilterLooksLikeSavedRgbGlass = value => String(value || '').includes('MOMusic-search-pill-glass-filter') && String(value || '').includes('saturate(1)');
...
function runElectronRuntimeCheck() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'MOMusic-quick-check-'));
...
`, 'utf8');
  ...
  try {
    const result = spawnSync(electron, [qaScript], {
      cwd: appRoot,
      env: { ...process.env, MOMusic_QA_APP_ROOT: appRoot, MOMusic_QA_PRELOAD: qaPreload },
      encoding: 'utf8',
      timeout: 45000
    });
    if (result.error) {
    ...
    const match = String(result.stdout || '').match(/MOMusic_QA_RESULT:(\{.*\})/);
...
function runMainStartupRecoveryCheck() {
  ...
  const runtimeName = `MOMusicStartupQA-${process.pid}-${Date.now()}`;
  const qaUserData = path.join(appData, runtimeName);
  const stateFile = path.join(qaUserData, 'startup-state.json');
  const qaSessionData = path.join('D:\\MOMusicCache\\chromium', runtimeName);
  try {
    const result = spawnSync(electron, [appRoot], {
      cwd: appRoot,
      env: {
        ...process.env,
        MOMusic_RUNTIME_NAME: runtimeName,
        MOMusic_APP_USER_MODEL_ID: 'com.MOMusic.startup.qa',
        MOMusic_NO_DESKTOP_SHORTCUT: '1',
        MOMusic_STARTUP_TEST_SERVER_DELAY_MS: '4500',
        MOMusic_STARTUP_TEST_FAIL_FIRST_NAV: '1',
        MOMusic_STARTUP_QA_HIDDEN: '1',
        MOMusic_STARTUP_QA_EXIT_MS: '700',
      },
      encoding: 'utf8',
      timeout: 40000,
    });
