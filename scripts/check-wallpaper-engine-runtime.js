...
function makeDwmSurfaceSpawn(records, commands, options = {}) {
  ...
  return (file, args, spawnOptions) => {
    ...
    queueMicrotask(() => {
      ...
      const sourceHandle = Number(String(spawnOptions.env.MOMusic_WE_DWM_SOURCE_ID || '').split(':')[1]) || 4242;
      child.stdout.write(`${JSON.stringify({
        ok: true,
        ready: true,
        hostWindowHandle: Number(spawnOptions.env.MOMusic_WE_DWM_HOST_WINDOW_ID) || 31337,
        sourceWindowHandle: sourceHandle,
        surfaceWindowHandle: 6262,
        desktopIconLayering: spawnOptions.env.MOMusic_WE_DWM_DESKTOP_ICON_LAYERING === '1',
      })}\n`);
    });
    return child;
  };
...
async function main() {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'MOMusic-we-runtime-'));
    ...
    desktopCapturer: {
      async getSources(options) {
        ...
        if (exposeGlassCaptureSource) {
          sources.push({ id: 'window:6262:0', name: 'MOMusic WE DWM Surface' });
        }
        return sources;
      },
    },
  ...
  assert(stalledLayeringCommands.includes('I|0') && stalledLayeringCommands.includes('Q'),
    'the fallback must request ordinary ordering and stop only the MOMusic DWM helper');
  ...
  await spacedControlRuntime._runTransientControl('C:\\Program Files\\Wallpaper Engine\\wallpaper64.exe', [
    ...
    'RAW~({"volume":0})~END',
    '-location',
    'MOMusic Wallpaper spaced-path-test',
  ]);
  ...
  assert(spacedControlCalls[0].args.includes('"MOMusic Wallpaper spaced-path-test"'));
  ...
  try {
    ...
    assert.strictEqual(signatureCalls[0].options.env.MOMusic_WE_SIGNATURE_TARGET, executable);
    ...
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_SOURCE_ID, 'window:4242:0');
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_HOST_WINDOW_ID, '31337');
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_SOURCE_EXECUTABLE, executable);
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_HOST_EXECUTABLE, executable);
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_DESKTOP_ICON_LAYERING, '1',
      'a desktop-icon layering request made while the helper is down must replay into its next start');
    assert.strictEqual(firstDwmSurface.options.env.MOMusic_WE_DWM_SESSION_ID, started.sessionId);
    ...
    assert(!dwmHelperSource.includes('MOMusic WE Glass Refraction'));
    ...
    assert.strictEqual(dwmSurfaceSpawns[1].options.env.MOMusic_WE_DWM_SOURCE_ID, 'window:4242:1');
    assert.strictEqual(dwmSurfaceSpawns[1].options.env.MOMusic_WE_DWM_DESKTOP_ICON_LAYERING, '1',
    ...
    muteCalls.forEach((mute) => {
      ...
      assert.strictEqual(mute.args[mute.args.indexOf('-location') + 1].replace(/^\"|\"$/g, ''), locationTitle, 'every mute control must use the unique MOMusic location');
    ...
    const invalidColdEngineRuntime = new WallpaperEngineRuntime({
      ...
      execFile: (file, args, options, callback) => {
        const target = String(options && options.env && options.env.MOMusic_WE_SIGNATURE_TARGET || '');
    ...
    try {
      ...
      assert(/^MOMusic Wallpaper [a-f0-9]{24}$/.test(muteRetryLocation));
    ...
    const brokerRuntime = new WallpaperEngineRuntime({
      ...
      desktopCapturer: {
        async getSources() {
          const openCall = brokerCalls.find((call) => /\bopenWallpaper\b/.test(call.options.env.MOMusic_WE_CONTROL_COMMAND_LINE || ''));
          if (!openCall) return [];
          const match = /-playInWindow\s+"([^"]+)"/.exec(openCall.options.env.MOMusic_WE_CONTROL_COMMAND_LINE || '');
    ...
    assert.strictEqual(brokerOpen.options.env.MOMusic_WE_CONTROL_TARGET, executable);
    assert.strictEqual(brokerOpen.options.env.TEMP, brokerOpen.options.env.MOMusic_NATIVE_TEMP_DIR, 'Add-Type helpers must use the stable native temp directory');
    assert.strictEqual(brokerOpen.options.env.TMP, brokerOpen.options.env.MOMusic_NATIVE_TEMP_DIR, 'PowerShell TMP must match the stable native temp directory');
    assert(/\bopenWallpaper\b/.test(brokerOpen.options.env.MOMusic_WE_CONTROL_COMMAND_LINE));
    assert(/wallpaper-engine-scene-stage/i.test(brokerOpen.options.env.MOMusic_WE_CONTROL_COMMAND_LINE), 'an embedded sound package must launch through the cached silent Scene manifest');
    ...
    brokerCalls.slice(1, 2).forEach((call) => {
      assert(/\bapplyProperties\b/.test(call.options.env.MOMusic_WE_CONTROL_COMMAND_LINE));
      assert(/-location\s+"MOMusic Wallpaper [a-f0-9]{24}"/.test(call.options.env.MOMusic_WE_CONTROL_COMMAND_LINE), 'audio suppression must target only the MOMusic pop-out location');
      assert(call.options.env.MOMusic_WE_CONTROL_COMMAND_LINE.includes('RAW~({"volume":0})~END'), 'the broker must preserve Wallpaper Engine RAW JSON without backslash escaping');
      assert(!/\s-control\s+mute(?:\s|$)/.test(call.options.env.MOMusic_WE_CONTROL_COMMAND_LINE), 'global Wallpaper Engine mute must never be used');
    });
    assert(/\bcloseWallpaper\b/.test(brokerCalls[2].options.env.MOMusic_WE_CONTROL_COMMAND_LINE));
    ...
    assert(/\bgetWallpaper\b/.test(readyBroker.options.env.MOMusic_WE_CONTROL_COMMAND_LINE));
    assert.strictEqual(readyBroker.options.env.MOMusic_WE_CONTROL_WAIT, '1', 'the readiness broker must wait for the control process to exit');
    assert.strictEqual(readyBroker.options.env.MOMusic_WE_CONTROL_WAIT_TIMEOUT, '10000');
