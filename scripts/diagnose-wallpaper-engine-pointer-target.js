...
const outputDirectory = path.resolve(outputArgument
  ...
  : path.join(os.tmpdir(), `MOMusic-we-pointer-target-${Date.now()}`));
...
$expectedTitle = [Environment]::GetEnvironmentVariable('MOMusic_WE_DIAG_SOURCE_TITLE', 'Process')
$targetMode = [Environment]::GetEnvironmentVariable('MOMusic_WE_DIAG_TARGET', 'Process')
$xUnit = [int][Environment]::GetEnvironmentVariable('MOMusic_WE_DIAG_X', 'Process')
$yUnit = [int][Environment]::GetEnvironmentVariable('MOMusic_WE_DIAG_Y', 'Process')
$source = @'
using System;
...
public static class MOMusicWePointerTargetProbe {
...
[MOMusicWePointerTargetProbe]::Run($expectedTitle, $targetMode, $xUnit, $yUnit) | ConvertTo-Json -Compress
`.trim();
}
...
function postPointer(target, xUnit, yUnit) {
  const stdout = execFileSync('powershell.exe', [
    ...
    env: {
      ...process.env,
      MOMusic_WE_DIAG_SOURCE_TITLE: sourceTitle,
      MOMusic_WE_DIAG_TARGET: target,
      MOMusic_WE_DIAG_X: String(xUnit),
      MOMusic_WE_DIAG_Y: String(yUnit),
    },
  });
  return JSON.parse(String(stdout || '').trim());
}

async function captureMOMusic(label) {
  const sources = await desktopCapturer.getSources({
    types: ['window'],
    thumbnailSize: { width: 1440, height: 810 },
    fetchWindowIcons: false,
  });
  const matches = sources.filter((source) => source.name === 'MOMusic');
  if (matches.length !== 1) {
    throw new Error(`Expected one MOMusic window capture source, found ${matches.length}`);
  }
...
async function captureAfter(target, side, xUnit) {
  ...
  await wait(settleMs);
  const capture = await captureMOMusic(`${target}-${side}`);
  return { posted, capture };
}

async function run() {
  if (!sourceTitle) throw new Error('Pass --source-title with the exact MOMusic Wallpaper session title');
  fs.mkdirSync(outputDirectory, { recursive: true });
  const baseline = await captureMOMusic('baseline');
