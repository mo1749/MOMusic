...
async function run() {
  ...
  try {
    const prepared = await client.evaluate(`(async () => {
      ...
      window.__MOMusicKeyboardProbe = { trustedPointerDown: false, pointerTarget: '', f13: 0 };
      document.addEventListener('pointerdown', (event) => {
        window.__MOMusicKeyboardProbe.trustedPointerDown = event.isTrusted === true;
        window.__MOMusicKeyboardProbe.pointerTarget = event.target && event.target.id || '';
      }, { once: true, capture: true });
      window.__MOMusicKeyboardProbeKeyListener = (event) => {
        if (event.code === 'F13') window.__MOMusicKeyboardProbe.f13 += 1;
      };
      document.addEventListener('keydown', window.__MOMusicKeyboardProbeKeyListener, { capture: true });
      return {
        status,
    ...
    const marker = `MOMusicKeyboardFocus-${Date.now()}`;
    ...
    const restored = await client.evaluate(`(() => {
      const input = document.getElementById('search-input');
      const probe = window.__MOMusicKeyboardProbe || {};
      const snapshot = {
        documentFocused: document.hasFocus(),
    ...
    assert.equal(restored.documentFocused, true,
      'trusted MOMusic pointerdown did not restore renderer keyboard focus');
    ...
    console.log(JSON.stringify({
      ...
      manualHardwareChecksStillRequired: [
        ...
        'After clicking a real desktop icon, click MOMusic again and test Space, arrows, and configured local hotkeys.',
      ],
    ...
    try {
      await client.evaluate(`(async () => {
        ...
        if (window.__MOMusicKeyboardProbeKeyListener) {
          document.removeEventListener('keydown', window.__MOMusicKeyboardProbeKeyListener, true);
          delete window.__MOMusicKeyboardProbeKeyListener;
        }
        delete window.__MOMusicKeyboardProbe;
