...
async function main() {
  ...
  try {
    ...
    const mediaUrl = (kind, id) => `MOMusic-wallpaper://${kind}/${id}?token=${snapshot.mediaToken}`;
    ...
    const missingToken = await instance.mediaResponse(new Request(`MOMusic-wallpaper://media/${video.id}`));
    assert.strictEqual(missingToken.status, 404);
    const malformed = await instance.mediaResponse(new Request(`MOMusic-wallpaper://media/%E0%A4%A?token=${snapshot.mediaToken}`));
