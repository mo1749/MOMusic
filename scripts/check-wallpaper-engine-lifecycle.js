...
const QA_PARENT = 'D:\\MOMusicCache\\we-lifecycle-qa';
const DEFAULT_EXE = path.resolve(__dirname, '..', '..', '..', 'MOMusic.exe');
...
const runtimeName = `MOMusic-WE-Lifecycle-QA-${runId}`;
...
const logPath = path.join(qaRoot, 'MOMusic.log');
...
function ensureQaDirectories() {
  ...
  assert(/^MOMusic-WE-Lifecycle-QA-[A-Za-z0-9-]+$/.test(path.basename(systemUserDataPath)), 'System QA userData name is not unique and safe');
...
function removeSystemUserDataJunction() {
  ...
  assert(/^MOMusic-WE-Lifecycle-QA-[A-Za-z0-9-]+$/.test(path.basename(systemUserDataPath)), 'Refusing to remove a non-QA system userData path');
  let stat;
...
public static class MOMusicWeLifecycleWindows {
...
$expected=[Environment]::GetEnvironmentVariable('MOMusic_QA_WINDOW_TITLE')
$items=[Collections.Generic.List[object]]::new()
$callback=[MOMusicWeLifecycleWindows+EnumWindowsProc]{
  param([IntPtr]$handle,[IntPtr]$state)
  $length=[MOMusicWeLifecycleWindows]::GetWindowTextLength($handle)
  if ($length -gt 0) {
    $text=[Text.StringBuilder]::new($length+1)
    [void][MOMusicWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity)
    if ($text.ToString() -ceq $expected) {
      [uint32]$ownerPid=0
      [void][MOMusicWeLifecycleWindows]::GetWindowThreadProcessId($handle,[ref]$ownerPid)
      $items.Add([pscustomobject]@{
        handle=$handle.ToInt64().ToString()
        pid=[int]$ownerPid
        visible=[MOMusicWeLifecycleWindows]::IsWindowVisible($handle)
        title=$text.ToString()
      })
    }
  }
...
[void][MOMusicWeLifecycleWindows]::EnumWindows($callback,[IntPtr]::Zero)
ConvertTo-Json -Compress -InputObject @($items)
`, { MOMusic_QA_WINDOW_TITLE: String(title || '') });
  if (!output) return [];
...
function topLevelWindowsByPid(processId) {
  const output = powershellEncoded(`${WINDOW_NATIVE_SOURCE}
$expected=[uint32][Environment]::GetEnvironmentVariable('MOMusic_QA_PROCESS_ID')
$items=[Collections.Generic.List[object]]::new()
$callback=[MOMusicWeLifecycleWindows+EnumWindowsProc]{
  param([IntPtr]$handle,[IntPtr]$state)
  [uint32]$ownerPid=0
  ...
  if ($ownerPid -eq $expected) {
    ...
    if ($length -gt 0) { [void][MOMusicWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity) }
    $items.Add([pscustomobject]@{
      handle=$handle.ToInt64().ToString()
      pid=[int]$ownerPid
      visible=[MOMusicWeLifecycleWindows]::IsWindowVisible($handle)
...
[void][MOMusicWeLifecycleWindows]::EnumWindows($callback,[IntPtr]::Zero)
ConvertTo-Json -Compress -InputObject @($items)
`, { MOMusic_QA_PROCESS_ID: String(processId || 0) });
  if (!output) return [];
  const value = JSON.parse(output.split(/\r?\n/).filter(Boolean).slice(-1)[0]);
  return Array.isArray(value) ? value : [value].filter(Boolean);
}
...
function windowHandleExists(handle) {
  const output = powershellEncoded(`${WINDOW_NATIVE_SOURCE}
$handle=[IntPtr]::new([long][Environment]::GetEnvironmentVariable('MOMusic_QA_HOST_HANDLE'))
if ([MOMusicWeLifecycleWindows]::IsWindow($handle)) { '1' } else { '0' }
`, { MOMusic_QA_HOST_HANDLE: String(handle || '') });
...
$handle=[IntPtr]::new([long][Environment]::GetEnvironmentVariable('MOMusic_QA_HOST_HANDLE'))
if (-not [MOMusicWeLifecycleWindows]::IsWindow($handle)) { throw 'QA host window no longer exists' }
$foreground=[MOMusicWeLifecycleWindows]::GetForegroundWindow()
[uint32]$foregroundPid=0
$foregroundThread=if ($foreground -ne [IntPtr]::Zero) { [MOMusicWeLifecycleWindows]::GetWindowThreadProcessId($foreground,[ref]$foregroundPid) } else { 0 }
$currentThread=[MOMusicWeLifecycleWindows]::GetCurrentThreadId()
$attached=$false
if ($foregroundThread -gt 0 -and $foregroundThread -ne $currentThread) {
  $attached=[MOMusicWeLifecycleWindows]::AttachThreadInput($currentThread,$foregroundThread,$true)
}
try {
  [void][MOMusicWeLifecycleWindows]::ShowWindowAsync($handle,9)
  [void][MOMusicWeLifecycleWindows]::SetWindowPos($handle,[IntPtr]::new(-1),0,0,0,0,0x43)
  [void][MOMusicWeLifecycleWindows]::BringWindowToTop($handle)
  [void][MOMusicWeLifecycleWindows]::SetForegroundWindow($handle)
  [void][MOMusicWeLifecycleWindows]::SetWindowPos($handle,[IntPtr]::new(-2),0,0,0,0,0x43)
  Start-Sleep -Milliseconds 120
} finally {
  if ($attached) { [void][MOMusicWeLifecycleWindows]::AttachThreadInput($currentThread,$foregroundThread,$false) }
}
`, { MOMusic_QA_HOST_HANDLE: String(handle || '') });
}
...
$callback=[MOMusicWeLifecycleWindows+EnumWindowsProc]{
  ...
  if ($length -gt 0) {
    ...
    [void][MOMusicWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity)
    if ($text.ToString() -ceq $expected) {
      [void][MOMusicWeLifecycleWindows]::PostMessage($handle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
    }
  }
  return $true
}
...
async function waitForCdpTarget(port, child, timeoutMs = 60000) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode != null) throw new Error(`MOMusic exited before CDP became ready (${child.exitCode})`);
    try {
  ...
  throw new Error(`Timed out waiting for MOMusic CDP: ${lastError && lastError.message || 'no page target'}`);
}
...
const STATE_EXPRESSION = `(async () => {
  ...
  return {
    ...
    videoTrackSettings,
    capturePath: stream && typeof stream.__MOMusicCapturePath === 'string'
      ? stream.__MOMusicCapturePath
      : '',
    cursorSuppressed: stream && typeof stream.__MOMusicCursorSuppressed === 'boolean'
      ? stream.__MOMusicCursorSuppressed
      : null,
...
async function waitForHostWindow(processId, timeoutMs = 20000) {
  ...
  throw new Error(`QA MOMusic host window was not found for PID ${processId}: ${JSON.stringify(windows)}`);
}
...
async function main() {
  ...
  assert(fs.existsSync(executablePath), `MOMusic executable not found: ${executablePath}`);
  ...
  try {
    ...
    const env = {
      ...
      MOMusic_APP_USER_MODEL_ID: `com.MOMusic.we-lifecycle-qa.${runId}`,
      MOMusic_NO_DESKTOP_SHORTCUT: '1',
      MOMusic_CREATE_DESKTOP_SHORTCUT: '0',
      ...(productionThrottling ? {} : { MOMusic_KEEP_BACKGROUND_RENDERING: '1' }),
    };
    ...
    assert(actualUserDataPath && fs.existsSync(actualUserDataPath), 'QA userData path was not reported by MOMusic');
    ...
    const initialTitle = `MOMusic Wallpaper ${initial.sessionId}`;
    ...
    const stopAllRaceTitle = `MOMusic Wallpaper ${afterStopAllRace.sessionId}`;
    ...
    const minimizeRestoreTitle = `MOMusic Wallpaper ${restoredFromMinimize.sessionId}`;
    ...
    const hideRestoreTitle = `MOMusic Wallpaper ${restoredFromHide.sessionId}`;
    ...
    const fullscreenTitle = `MOMusic Wallpaper ${fullscreen.sessionId}`;
    ...
    const fullscreenRestoreTitle = `MOMusic Wallpaper ${restoredFromFullscreen.sessionId}`;
    ...
    assert(exit, 'QA MOMusic did not exit within 20 seconds');
    cleanExit = true;
    ...
    assert.strictEqual(windowHandleExists(hostHandle), false, 'QA MOMusic host window survived process exit');
