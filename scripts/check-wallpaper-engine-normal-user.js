...
const outputRoot = path.join('D:\\MOMusicCache\\we-normal-user-qa', runId);
...
async function waitForCdpTarget(timeoutMs = 60000) {
  ...
  throw new Error(`Timed out waiting for MOMusic CDP on ${port}: ${lastError && lastError.message || 'no page target'}`);
}
...
function listExactSourceWindows() {
  return listWindows('MOMusic Wallpaper ');
}

function listDwmSurfaceWindows() {
  return listWindows('MOMusic WE ');
}
...
async function waitForExactSourceWindowCount(expected, timeoutMs = 20000) {
  ...
  throw new Error(`Exact MOMusic Wallpaper window count stayed ${windows.length}, expected ${expected}`);
}
...
function assertControlGlassGeometry(state, nativeWindows, round) {
  ...
  assert(/url\(["']?#MOMusic-control-glass-filter["']?\)/.test(bar.backdropFilter),
  ...
  const base = nativeWindows.find((item) => item.title === 'MOMusic WE DWM Surface');
  assert(base && base.rect && base.visible, `round ${round}: base DWM window is missing or hidden`);
  assert(!nativeWindows.some((item) => item.title === 'MOMusic WE Glass Refraction'),
...
function assertNativeWindows(state, round) {
  ...
  assert.strictEqual(exactWindows[0].title, `MOMusic Wallpaper ${state.sessionId}`,
  ...
  const baseWindows = dwmWindows.filter((item) => item.title === 'MOMusic WE DWM Surface');
  const glassWindows = dwmWindows.filter((item) => item.title === 'MOMusic WE Glass Refraction');
